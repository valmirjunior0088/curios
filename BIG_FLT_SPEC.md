...
- **Kernel conversion handles stuck recursive calls** (fixed in 76c870f9, forced by Part 2's corpus): applications of `rec`-bound names whose unfolding stays a stuck match are normal forms (match-guarded delta); rec bodies are opened once and memoized under stable labels; conversion falls back to lazy delta to identify folded-vs-unfolded spellings of the same call. Conversion is therefore the reliable mechanism for closing goals over stuck recursive applications — lean on it. The match-guarded delta only protects names marked recursive: `erase_module` seeds its own fresh context and was a fifth rec-definition site missing the mark (fixed alongside Part 3), which made erasure of Part 3's corpus diverge on exactly the pre-76c870f9 pathology.
- **Elaborator proof idioms** (Part 2's corpus is the reference): explicit-subject reflexivity (`qed(x)`) where `Eq/refl()`'s implicit meets reducible indices; explicit `@`-arguments where a helper's constructor pattern meets a reducible premise index; nested single-scrutinee matches with motives (tuple scrutinees do not refine goals); congruence lemmas as dependent matches on the equation. To make a stuck match reduce counterfactually, convoy on the scrutinee itself — reliable only when the matched application's arguments are plain variables; generalize a composed argument through a helper lemma and let conversion instantiate it. Match-rewrites on `Eq` proofs whose rewritten occurrence is produced by reduction are elaboration-order-fragile — build the equation with congruence + `Eq/trans` chains instead. Two additions from Part 3's corpus: (1) a goal or ascription containing a *partially-reduced match* — an `add`/`mul`-style pair-match applied to one constructor and one stuck argument — blocks bidirectional propagation, and every implicit-bearing call under it (`Eq/trans`, `Eq/sym`, congruence helpers) then needs explicit `@`-arguments; write long chains as pairwise let-ascribed accumulators (`t1 = trans(s1, s2)`, `t2 = trans(t1, s3)`, …) so no `trans` ever sits in another `trans`'s argument slot. (2) Never inline a derived proof as a witness-extractor argument (`pos_lt_witness(y, x, pos_cmp_gt_to_lt(x, y, w))`): the projected witness then drags the whole proof term through every reduction and blows the type-checking budget — bind the flipped premise as a variable first, or better, state each trichotomy arm as its own lemma taking the verdict evidence as a parameter (the `mul_pos_sub_eq`/`_lt`/`_gt` shape, with `gt` derived from `lt` by antisymmetry instead of a second witness extraction).
...
## Part 3 — `BigInt`: signed integers and the sign-case proofs (delivered)
...
- Ops: `add` via signed positive subtraction — `pos_sub(p, q) : BigInt`, a nine-row recursion over the doubling helpers `dbl`/`dbl_succ`/`dbl_pred` returning the signed difference directly (Coq's `Z.pos_sub`) — so no op consults an order test it then has to reason about separately; `sub` = `add` of `neg`; `mul` (sign by constructor case, magnitudes via `pos_mul`); `neg`/`abs`/`magnitude`; `Order`-valued `cmp` (`pos_cmp` flipped under `neg`) + `Bln` `lt`/`lte`/`gt`/`gte`; `eql` via `cmp`; `of_nat`/`of_bignat`, `of_int` (the i31 boundary helper, via `Int/abs`; note `Int/abs` of the carrier minimum `-2^30` traps, so that one value is unreachable); `is_even`/`div2`; `to_str` (Int-style explicit sign, zero renders `"+0"`).
- Ring laws as delivered: the `pos_sub` spine (`pos_sub_diag`, `pos_sub_succ`, the mutual `pos_sub_add_l`/`pos_sub_succ_add` pair for the carry rows, antisymmetry `pos_sub_flip`, the witness transports `pos_sub_of_lt`/`pos_sub_of_gt`), the decomposition laws L1 `pos_sub(p+q, r) = pos(p) + pos_sub(q, r)` and L2 `pos_sub(x, y+z) = pos_sub(x, y) + neg(z)` (each a single `pos_cmp` trichotomy over `pos_sub_cancel`, the common-addend law), and on top of those: `add_zero_r`, `add_neg_l/r`, `neg_neg`, `neg_add`, `add_comm`, `add_assoc` (27 arms, every mixed-sign case a pure L1/L2 chain), `add_cancel_l/r`, `mul_comm`, `mul_assoc`, `neg_mul_l`, M1 `mul(pos(p), pos_sub(x, y)) = pos_sub(p·x, p·y)`, `distrib_l/r`, `mul_cancel_l/r` (stated per positive scalar, via the BigInt no-confusion toolkit + `pos_mul_cancel_l`), and the order laws `cmp_refl`/`cmp_eq`/`cmp_flip`/`cmp_trans` (antisymmetry = `cmp_flip` + `cmp_eq`, since the carrier is canonical). The sign splits happen here, once, and nowhere else.
...
3. Part 3 (`BigInt`) — **delivered**, together with the erase-phase completion of the 76c870f9 kernel fix it forced (`mark_recursive` in `erase_module`).
