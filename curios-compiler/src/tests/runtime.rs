...
fn let_bang_std_parse_threads_bangs_left_to_right() {
    ...
    let loader = curios::text::FileLoader::new(base);
...
fn folds_constant_arg_through_let_function() {
    ...
    curios::compile_entrypoint(
        Duration::from_secs(10),
        &entrypoint,
        &text::NullLoader,
        |stage| {
            if let curios::Stage::ContOptm(module) = stage {
...
fn printf_partial_evaluation_reduces_residual() {
    ...
    let wasm_module = curios::compile_entrypoint(
