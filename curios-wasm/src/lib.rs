//! The compiler's wasm-GC target representation: an in-memory module builder and its binary encoder, the final stage of the pipeline. `curios-emit` lowers continuation IR into a [`Module`], and [`to_bytes`] produces the binary that wasmtime (`curios-runtime`), the browser (`curios-js`), and `wasm-opt` (`curios-binaryen`) consume.
//!
