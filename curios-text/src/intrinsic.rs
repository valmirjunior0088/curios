...
/// The hardcoded intrinsic vocabulary, shaped after `curios_core::Intrinsic` rather than after the written syntax (`Nat`'s payload still differs, keeping surface-only detail like radix and spread segments — `Int`/`Flt` share `Int`/`Flt` verbatim, since neither carries any surface-only presentation state): the scalar carriers (`Bool`/`Nat`/`Int`/`Flt`) with their type formers, literal values, and operators, the conversions between them, the packed `Bits`/`Bytes` and `List` free-monoid carriers with their literals and accessors, `Handle` with the store-described `Foreign` host calls, and the `Cell` mutable reference. The parser only ever produces the literal-value variants (`Bool`/`Nat`/`Flt`/packed binary/`List`); every other variant — type formers and operations alike — is baked directly into the embedded `sys` roster's definition bodies (see `sys_module`), so user code reaches them as ordinary named bindings rather than syntax.
#[derive(Debug, Clone, PartialEq)]
pub enum Intrinsic {
    BoolType,
    Bool(bool),
