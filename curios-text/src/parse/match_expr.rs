...
// A choose condition arm `| cond => body`. The catch spans through `=>` so a
// condition term that merely *starts* like the default backtracks cleanly.
// A bare `_` parses as an ordinary `Name` term, so the choose's `| _ =>`
// default would otherwise be swallowed here; the `flat_map` guard rejects a
// lone `_` condition, letting `many0` stop and hand the default to
// `parse_choose_default`. A condition that merely *begins* with `_` (e.g.
// `_ready`) has a longer head and passes the guard, parsing as an ordinary
// condition.
...
// One choose arm: a bind arm first (its `catch` backtracks when the pattern is
...
// The mandatory `| _ =>` default arm closing a `choose`. A `Bool` ladder is a
// dispatch form (it enumerates no shapes), so `_` is required, not optional.
...
// `choose | test => body | … | _ => default end`. No head term — the arms are
// `Bool` conditions or refutable binds, tried top-to-bottom, first to fire
// wins. Distinguished from `match` by keyword alone, so there is no
// disambiguation to speak of here.
...
// A `match` is exactly one surface shape now: the general headed pattern
