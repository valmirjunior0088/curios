...
pub struct FuncParam {
    pub plicity: Plicity,
    pub pattern: Pattern,
    /// `None` is the surface `(x) => …` form, sugar for `(x : _) => …`; it lowers to a hole, solved by `curios_elab::elaborate` against the expected function type when the lambda is checked, or synthesized from the annotation when inferred.
...
/// One tuple-literal / struct-literal field as written: an optional name annotation (`(status = 0, handle = h)`) and the field value. Names are checked positionally against the expected tuple type's labels at elaboration and never survive past it.
#[derive(Debug, Clone, PartialEq)]
...
impl TupleField {
    /// The field's value with the definition sugar undone: the written value when the field is plain, the lambda `(params) => value` when it was written `label(params) = value`.
    pub(crate) fn desugared_value(&self) -> Term {
        match &self.func_params {
            // This sugar's binders stay plain-name-only (it is not one of the pattern-accepting sites) but each retains its plicity mark, so a `@`/`use` written in the sugar is carried onto the generated lambda's slot rather than dropped.
...
/// One tuple-pattern / struct-pattern field: a labeled sub-pattern (`label = pattern`) or a bare positional one. The literal mirror of [`TupleField`] with `Term` replaced by `Pattern` in the value slot — construction and destructuring read as inverses of each other.
#[derive(Debug, Clone, PartialEq)]
...
/// An ordered guarded ladder: `choose | test => body | … | _ => default end`. There is no scrutinee — the arms are tried top-to-bottom and the first that fires selects its body; the mandatory final `| _ =>` supplies the fallthrough. An arm's [`ChooseTest`] is either a `Bool` condition (`| cond =>`) or a refutable bind (`| pattern = value =>`, Rust `if let`). Lowering right-folds the ladder into nested `Bool` matches / single-row matrix matches (`into_core::match_compile`'s `ladder_region`), so each body inherits the definitional refinement of its condition for free (the same `refine_head` keying a hand-written `match cond | true => … | false => …` would get). A dispatch form, not an elimination — it consumes no scrutinee, so it is not a `match` at all: it enumerates no shapes, so the `_` default is mandatory (as in a headed `Nat` literal dispatch). No motive.
#[derive(Debug, Clone, PartialEq)]
pub struct Choose {
    /// The arms, in source order; the first whose test fires selects its body. May be empty (then the ladder is just its `default`).
    pub arms: Vec<ChooseArm>,
    pub default: Term,
}

/// One arm of a [`Choose`] ladder: a [`ChooseTest`] and the `body` it selects when the test fires.
#[derive(Debug, Clone, PartialEq)]
...
pub enum ChooseTest {
    ...
    Cond(Term),
    /// `| pattern = value =>` — a refutable bind (Rust `if let`): the arm fires when `value` matches the (refutable, possibly nested) `pattern`, binding its sub-patterns in the body. A bare-[`MatchPattern::Binder`] `pattern` is irrefutable and rejected by lowering — it would be a plain `let`.
    Bind { pattern: MatchPattern, value: Term },
}

/// A projection names its field either positionally (`p.0`) or by the tuple type's label (`p.status`). Labels are resolved to positions during core elaboration; they never survive past it.
#[derive(Debug, Clone, PartialEq)]
pub enum Field {
    Index(usize),
    Label(String),
...
pub struct Match {
    pub head: Term,
    /// The written motive, a term checked against the eliminator's motive type `(ī : Ī(p̄)) -> I(p̄, ī) -> Sort` — ordinarily a lambda binding the scrutinee's indices and then the scrutinee. `None` records that no `:` was written, and lowering mints a silent metavariable for it.
    pub motive: Option<Term>,
    /// The arms, in source order. Each pairs a (possibly nested, across constructors/tuples/structs — see [`MatchPattern`]) pattern with its body; zero arms is legal (a vacuous elimination, e.g. of `False`). The grammar enforces "full enumeration" (no wildcard/catch-all, no row priority — see `into_core::match_compile`'s doc comment); lowering rejects a repeated tag and an overlapping/duplicate row.
...
/// A match-arm pattern: genuinely refutable, unlike [`Pattern`] (which is always irrefutable — see its own doc comment). Distinct from `Pattern` on purpose, even though `Tuple`/`Struct` mirror its field grammar exactly: a `let`/lambda/function-sugar binder site never dispatches on shape, but a match arm's whole point is dispatching on shape, so the two must never be conflated.
#[derive(Debug, Clone, PartialEq)]
...
/// One tuple-pattern / struct-pattern field in a [`MatchPattern`]: a labeled sub-pattern (`label = pattern`) or a bare positional one. The literal mirror of [`PatternField`] with `Pattern` replaced by `MatchPattern`.
#[derive(Debug, Clone, PartialEq)]
...
pub struct LetBinding {
    /// The bound pattern — `let x = …`, `let (x, y) = …`, or the `f` in the `let f(…) -> R = …` function-definition sugar. A compound pattern here is grammatically legal for the function-sugar form too, but pointless: a function's own binding is never itself a tuple/struct value, so destructuring it fails the same "not a tuple/struct" projection type error a hand-written misuse would (no special-casing needed).
    pub binder: Pattern,
