...
//! - `trace` — present under `enabled`, so it carries no link here — runs a closure under a subscriber that writes one row per span and event as it happens, without touching the process-global subscriber. It is the only producer: profiling is configured where it is used, in code.
