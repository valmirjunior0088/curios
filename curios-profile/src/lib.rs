...
//! - `capture` — present under `enabled`, so it carries no link here — runs a closure under a thread-local subscriber and returns aggregate per-span timings, without touching the process-global subscriber. It is the only consumer: profiling is configured where it is used, in code.
...
//!
//! Why the dependency is named here and nowhere else, why profiling is configured in code and never from the environment, why a third instrument measures magnitude beside time and bytes, and why the allocator counts process-wide are `README.md`'s decisions.
//!
