...
///
/// One bundle rather than five parameters, because the five are one subject. The two names decide the *address* and the three entry-side facts decide the *record*, which is the same split the whole scheme rests on. The engine is not among them: it is a fact about this process rather than about the program, computed once by the crate that owns the runtime and handed to [`Verdicts::payload_get`] and [`Verdicts::payload_put`] beside this.
...
impl Verdicts {
    ...
    pub fn payload_get(
        ...
        let filed = fs::read(self.store.payload(&slot(self, program, &placed, engine)?)).ok()?;
    ...
    pub fn payload_put(
        ...
        let Some(slot) = slot(self, program, &placed, engine) else {
            return;
        };
...
/// The slot `program` is filed in after `placed` for `engine`, or `None` when it may not be filed at all — which is when the compiler cannot identify itself, and for the reason `Verdicts::compiler` gives.
fn slot(
    verdicts: &Verdicts,
    program: &Program<'_>,
    placed: &[Placed],
