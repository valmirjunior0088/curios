...
- A file target is placed in the unit that declares it: a module under a package's directory is analysed as that package's library, an executable's entry as that executable, anything else standalone. A file under `curios-prelude-archive/std/` is the exception with no answer: its package claims `/std`, which the archived prelude already mounts, so the question is refused as a prefix collision. Ask about `/std` through a program that imports it.
...
- Production compilation has no fixed-prelude source fallback or cache-miss branch. Archive construction or restoration failure is a compiler invariant and fails loudly. The archived prelude is in scope unconditionally: `curios-pipeline`'s `standard` module puts every prelude root in the scope whatever the units beside it declare, so a unit claiming a prefix the prelude mounts collides with it and is refused, exactly as two source units claiming one prefix are. Nothing supersedes an archived unit, which is why a question about a prelude module is asked of the archive rather than by recompiling `/std` from source.
