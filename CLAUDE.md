...
- Production compilation has no fixed-prelude source fallback or cache-miss branch. Archive construction or restoration failure is a compiler invariant and fails loudly. A *question* about a prelude module is the one path that compiles `/std` from source, and it is not a fallback: the module is placed in its own package by `Membership`, and `curios-pipeline`'s `standard` module drops the archived unit of a prefix a source unit claims.
