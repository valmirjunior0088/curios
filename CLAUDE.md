...
- A file target is placed in the unit that declares it: a module under a package's directory is analysed as that package's library, an executable's entry as that executable, anything else standalone. A file under `curios-prelude-archive/std/` is answered only from the checkout that built the compiler: its package claims `/std` from the directory the archived unit's record names, so the fold withholds that root and compiles the package over the archived unit as its baseline. From any other tree the package claims a prefix the prelude mounts and is refused as a collision; ask about `/std` there through a program that imports it.
...
| The documentation record or its pages | `curios-document`'s `record.rs` and `pages.rs` with the Askama templates and static files beside them, the builder in `curios-text/src/into_core/document.rs` and its tests in `curios/src/tests/document.rs`, the engine in `curios-wonder/src/document.rs`, the store's verdict schema tag when the record's layout changes, `documentation/usage.md`'s Documenting, and `documentation/syntax.md`'s comments section when the `-- \|` grammar moves |
...
- `/sys` and `/std` are owned by `curios-prelude-archive` and compiled into one rkyv image of a `Unit` each, filed at `curios-prelude-archive/.artifacts/sys.rkyv` and `.../std.rkyv`. They are two units in that order — `/sys` names nothing above it, `/std` names `/sys` — each image a stored unit as a store slot holds one, with the record of the tree it was built from ahead of it; `curios document` names `std.rkyv`, the half whose documentation record it renders. Every `/std` source module is registered in its Curios index.
- Production compilation has no fixed-prelude source fallback or cache-miss branch. Archive construction or restoration failure is a compiler invariant and fails loudly. The archived prelude is in scope unconditionally, with the one exception the compiler's own record licenses: `curios-pipeline`'s `standard` module puts every prelude root in the scope whatever the units beside it declare, so a unit claiming a prefix the prelude mounts collides with it and is refused, exactly as two source units claiming one prefix are — unless it claims that prefix from the tree the archive records it was built from, in which case `standard` withholds the root and hands the archived unit over as the unit's baseline. Nothing else supersedes an archived unit.
