...
| Printer combinators | `curios-print` | The Wadler document algebra every IR's `Display` impls are written in. Depends on nothing — it is defined over `std::fmt` alone, which is what lets every IR crate use it without depending on each other |
...
One namespace is kept for a different reason, and it is the only one: `curios-runtime`'s `pub mod test_support`. Nothing in it collides with anything — the path exists so that `curios_runtime::test_support::GuestInstance` announces at its use site that the caller reached for scaffolding rather than product API. A namespace earns its place by *marking* code, never by separating names.
