...
- `/sys` and `/std` are owned by `curios-prelude-archive` and compiled into one rkyv image of a `Unit` each, filed at `curios-prelude-archive/.artifacts/sys.rkyv` and `.../std.rkyv`. They are two units in that order — `/sys` names nothing above it, `/std` names `/sys` — and `curios document` names `std.rkyv`, the half that carries a record. Every `/std` source module is registered in its Curios index.
