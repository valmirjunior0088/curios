...
| The documentation record or its pages | `curios-text/src/into_core/document.rs` and its tests in `curios/src/tests/document.rs`, the engine in `curios-wonder/src/document.rs`, the renderer in `curios/src/document.rs` with its Askama templates under `curios/templates/` and the static files under `curios/static/`, the store's verdict schema tag when the record's layout changes, `documentation/usage.md`'s Documenting, and `documentation/syntax.md`'s comments section when the `-- \|` grammar moves |
