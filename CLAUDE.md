...
**CRASH_COURSE.md** is a guided introduction aimed at Rust programmers. It walks through bindings, lambdas, `Nat.fold`, primitive types, tuples, atoms, the sum-type idiom via dependent tuples, dependent function types, and length-indexed vectors — always pairing Rust and Curios code side by side.

**SYNTAX.md** is the complete language reference. It covers lexical basics, top-level declarations (`let`, `rec`, `def`, `mod`, `use`), all term and type forms, literals, every primitive operation for `Nat`, `Int`, `Flt`, `Bin`, `Arr`, and `Sys`, and the canonical idioms for sum types and recursive types.
