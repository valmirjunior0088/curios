...
- No crate below `curios-prelude-archive` may spell a `/syn` name: `curios-utilities` states slots, `curios-prelude-archive/src/syntax.rs` states spellings, and the prelude build checks every slot against the sources. The one exception is `curios-analysis`'s `fixture`, the registry stand-in both checkers' tests are built against: it spells `/syn` names knowingly, and its `test-support` gate is what keeps them out of every build that ships. The erased runtime carriers for compiler-emitted literals are `Nat` and packed `Bytes`.
...
- A test's name is a sentence stating what is true — `an_unsatisfiable_universe_context_is_refused`, never `unsatisfiable_universe_context` — and repeats nothing its file or directory already says. The exception is a measurement: it reports a cost rather than asserting one, so it names its instrument (`field_shape_census`, `map_wall_spines_slope`) and keeps that name, because the recorded command beside it names the test. Where the same proposition is put to both checkers the two tests share one name deliberately; rename both or neither.
