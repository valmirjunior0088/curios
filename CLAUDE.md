...
**The binary is the tree's.** `cargo run --package curios --` runs the compiler this checkout builds, which is the one a change is being made to; it needs `make curios` once per checkout for the launcher it embeds. An installed `curios` on `PATH` is a different build and answers for it, not for the tree.
...
The native compiler embeds the slim `curios-runtime` launcher with `include_bytes!`, and that launcher must be built in its own Cargo invocation. `make curios` does both stages in order; `make curios/runtime` is the first stage alone, building `curios-runtime` without workspace feature unification so Cranelift and Binaryen stay out, and copying the launcher to `curios/.artifacts/<triple>`. A `curios-runtime` binary from a workspace build is not evidence the isolated launcher is slim. Building `curios` without that stage fails naming the command to run.
...
- `curios-js` uses plain `cargo build` plus `wasm-bindgen-cli`; do not introduce `wasm-pack` or `wasm-opt` without a design decision. Binaryen optimization belongs to the native product only.
