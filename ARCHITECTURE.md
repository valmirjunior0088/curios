...
Two deliberate lines: flex–flex pairs with distinct heads are never intersected — they park and either resolve through later pins or drain as errors; and parked problems drain **per item** — top-level items carry explicit types, so a later item never determines an earlier item's elaboration, and an unresolvable constraint is always attributed to its own definition.
