...
src/core.rs          facade; re-exports scope, int, flt, nat, prim, names, term, module, inductive, reduce, context, convert, error, typing, invert, elaborate, erase, zonk
...
| `Metavar`                      | A contextual metavariable occurrence: id, optional implicit-insertion provenance, and a spine (see below)                                              |
...
`erase_module(context, module, expected_type)` (in `erase.rs`) lowers the meta-free module to the `ersd::Module`, _trusting_ elaboration: nothing is re-checked — type information is re-derived only where lowering needs it (a lambda's parameter types from the checked function type, an application's argument types by re-inferring the head), and a shape that elaboration already ruled out is a compiler bug (`unreachable!`), not a user error.

`typing.rs` holds the shared judgment infrastructure — the `expect`/`refine_head` helpers, the parked-constraint retry/drain loop, and the timeout-aware `reduce_with`/`convert_with` wrappers. The `Error` enum lives in `error.rs`.

### Normalization (`reduce.rs`, `convert.rs`)

`reduce(context, term)` performs full beta-normalization: applies functions, eliminates lets, runs all primitive operations on concrete values. The deadline is an `Instant` set once at `Context::new(timeout)` and shared by every `reduce`/`convert`/`infer`/`erase` call through that context — the timeout bounds total work, not per-call work; when it expires, `Preempted` propagates up and surfaces as an `Error::ReducePreempted`.

`convert(context, type_, this, that)` checks definitional equality at a type by reducing both sides and comparing structurally. Uses a BFS queue with a `HashSet`-based visited set to avoid stack overflow on deeply nested terms.

### The elaboration context (`context.rs`)

The lexically-scoped part is per-frame stacks — **assumptions** (name → type), **definitions** (name → value), and the suppressible **counterfactual refinements** match arms register via `refine_head` — pushed and popped by `with_frame(f)`. Alongside them sit the frame-independent stores: the metavariable store (birth telescopes, identity spines shared per Γ-version, solutions with a rollback journal), the parked-constraint store (see the elaboration section above), the inductive registry, and a reduction cache. A shared `Entropy` counter (see [Utility layer](#utility-layer)) generates fresh names during type checking.
...
- **`OsHost`** maps the stdin/stdout/stderr handles onto the real process streams (raw `Read::read` for stdin, so short reads behave POSIX-style) and backs file handles with `std::fs::File`.
- **`MockHost`** routes writes (stdout and stderr alike) through an `mpsc::Sender<Vec<u8>>` and serves stdin reads from an `mpsc::Receiver<Vec<u8>>` pre-loaded with input lines: each message is one line, a `\n` is appended on refill, and an internal leftover buffer serves `count`-byte slices so short reads never drop bytes. File handles are backed by an in-memory filesystem. Constructors: `MockHost::in_out(lines)` returns the host plus the matching output `Receiver`; `MockHost::out()` is the input-empty shorthand; `MockHost::with_fs(lines, files)` additionally pre-seeds the in-memory filesystem and returns it for post-run inspection.

Up to eight operations are wired as Wasmtime host imports under `"env"` by `run_wasm`: the four conversions (each emitted only if the codegen `Table` records the corresponding `_used()` flag) route directly to the free functions; `io_open`/`io_close`/`io_read`/`io_write` route to the trait. Scalar _results_ of the io imports cross the boundary pre-boxed as i31 refs (so generated code can land them directly in anyref block params); a two-result import's resume block packs `(status, payload)` into the `{status, …}` record the prim's type promises. The `io_*` imports correspond to the `Tail::Host` variants introduced in [Stage 5](#stage-5--cps-lowering-srcersdto_cont).
...
| `src/entropy.rs`        | `Entropy<T>` — the shared gensym source: a `Cell`-backed monotonic counter whose `fresh()` mints a `T: Mint` (raw `usize` ids by default). Every fresh-name need in the pipeline draws from one: metavariables and binder labels in `text/to_core`, fresh names and the Γ-version stamp in the core `Context`, value/block/closure names in CPS lowering, synthesized bindings in `optm/evaluate_pure_calls`, and WASM codegen locals. |
