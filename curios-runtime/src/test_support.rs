...
//!
//! So this module hands out two opaque types instead. [`GuestValue`] carries a wasm value the caller cannot inspect except as an `i32` — which is what lets a test thread an opaque GC reference from one call into the next, the thing a scalar-only API could not express — and [`GuestInstance`] owns the store and the instance together, so no lifetime or borrow of wasmtime's escapes.
//!
