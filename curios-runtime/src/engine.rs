...
pub fn shared_engine() -> &'static Engine {
    static ENGINE: LazyLock<Engine> = LazyLock::new(|| {
        ...
        // Sixteen mebibytes, because the engine's own growth policy only ever reacts to a single allocation not fitting after a collection — so under death-birth churn the heap parks within a doubling of the live set and the collector recopies the survivors continually. Sixteen is the smallest measured size on the good side of that knee for both churn-class workloads, and the cold-page tax that argues for smaller never registers until far above it; commit is lazy, so a program that allocates little touches little. The figures and the retake recipe live with `chain_collection_decomposition` and `spines_collection_decomposition`; the decision is documentation/design/toolchain/the-heap-is-sized-ahead-of-its-churn.md.
