...
See [SYNTAX.md](documentation/SYNTAX.md) for the full language reference.
...
A Cargo workspace of twelve crates, layered along the pipeline: **`curios-abi`** (host/guest wire ABI constants) and **`curios-base`** (spans, entropy, the `name!` macro, parser/printer monads, and grain-aware packed Bits/Bytes windows) are the shared foundations; **`curios-wasm`** → **`curios-cont`** → **`curios-ersd`** → **`curios-core`** → **`curios-text`** are the pipeline stages (`text` → `core` → `ersd` → `cont` → `wasm`, each its own crate, code dependencies running the opposite direction of data flow); **`curios-pipeline`** is the pure driver chaining those stages (`compile_entrypoint`); **`curios-js`** is a `wasm-bindgen` build of `curios-pipeline` for a browser playground; **`curios-binaryen`** is FFI to Binaryen, built from a source release fetched and compiled by `build.rs` (no vendored source); **`curios-rt`** is the runtime-only engine + launcher stub; and **`curios`** is the driver + CLI, the only crate that links Cranelift and Binaryen. The standard library (`std`/`syn`) lives under **`curios-text`**, not `curios`.

For a full tour of the architecture, build/test workflow, and conventions, see [AGENTS.md](AGENTS.md). For the language itself, see [SYNTAX.md](documentation/SYNTAX.md). For what's built vs. planned, see [ROADMAP.md](documentation/ROADMAP.md).
