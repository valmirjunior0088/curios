//! Exact scalar semantics of the erased numeric carriers — `Nat` as `u32`,
//! `Int` as `i32`, `Flt` as binary32 — shared by every stage's constant
//! folder so their arithmetic cannot drift. The runtime's i31 envelope
//! appears nowhere in this module: a value the backend cannot box traps at
//! the Wasm boundary instead of changing.
//!
//! Only operations with semantic freedom live here — wrapping-versus-
//! saturating choices, trap conditions, fold-decline conditions. Comparisons
//! and plain bitwise operations have exactly one meaning and stay as native
//! operators at their use sites.
...
/// `Int` 32-bit left rotation of the carrier bits; the count is taken
/// modulo 32.
pub fn int_rotl(value: i32, shift: i32) -> i32 {
    (value as u32).rotate_left(shift as u32) as i32
}

/// `Int` 32-bit right rotation of the carrier bits; the count is taken
/// modulo 32.
...
/// `Int` remainder; traps on a zero divisor. `i32::MIN % -1` is `0` and does
/// not trap.
...
/// Truncate to `u32`; `None` traps outside `(-1, 2^32)` or on a non-finite
/// input.
...
/// Truncate to `i32`; `None` traps outside `[-2^31, 2^31)` or on a non-finite
/// input.
...
/// Binary32 minimum; `None` declines the fold on a NaN operand (Rust and
/// Wasm disagree on NaN propagation), which is not a trap — the runtime
/// operation proceeds.
pub fn flt_min(left: f32, right: f32) -> Option<f32> {
    (!left.is_nan() && !right.is_nan()).then(|| left.min(right))
}

/// Binary32 maximum; `None` declines the fold on a NaN operand, which is not
/// a trap — the runtime operation proceeds.
