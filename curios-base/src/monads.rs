//! The two combinator DSLs, and the one part of this crate that is *not* flattened into the root. Both are monads and both name their unit `pure`, so a flat re-export would leave `curios_base::pure` ambiguous at every use site — while `printer::pure` and `parser::pure` are each unambiguous and tell the reader which DSL a `Display` impl or a grammar rule is written in. Keeping them namespaces is what lets the shared vocabulary coexist, for the same reason `curios_core::kernel` stays a namespace beside the elaborator's like-named judgments.
