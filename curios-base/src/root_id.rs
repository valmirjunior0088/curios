...
impl RootId {
    /// The `RootId` a qualified name's leading path segment names. Any
    /// segment that isn't one of the three embedded roots is the entry
    ...
    /// each privileged (may reference an internal root); the entry program is
    /// ordinary.
