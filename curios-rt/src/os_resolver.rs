...
/// A finished lookup the worker hands back through the slot: the resolved
/// address blobs and the status reporting whether the name resolved at all.
/// Built only via [`Resolved::found`] / [`Resolved::not_found`], which keep the
...
impl Resolved {
    /// The outcome of a successful `getaddrinfo`: `Ok` with the resolved blobs,
    /// or `NotFound` when the name resolved to an empty set.
...
/// by the host's `resolve`. Empty until the worker completes; the [`Resolved`]
...
impl Slot {
    ...
    /// Worker side: deposit the finished lookup's result. Called exactly once,
    /// just before the worker wakes the poller.
...
/// The worker's half of a lookup's wakeup pipe: the write end whose single byte
/// makes the host's polled read end fire. The matching read end travels to the
/// host as [`Pending::ready`].
struct Signal {
    fd: OwnedFd,
}

impl Signal {
    /// Open a wakeup pipe, returning the host's read end and the worker's
    /// `Signal`. `std::io::pipe` sets `CLOEXEC` on both ends, so a forked child
    /// never inherits them.
    ...
    /// Wake the host's poller. The byte's *value* is irrelevant: the read end is
    /// only ever polled for readability and dropped, never `read`, so any single
    /// byte serves as the edge — `&[1]` is just a conventional one-event marker.
    /// A closed read end (the guest cancelled and dropped the handle) makes this
    /// `EPIPE`, which is fine — the result is just discarded. The Rust runtime
    /// ignores `SIGPIPE`, so the write returns an error rather than killing the
    /// process.
    fn signal(&self) {
...
/// the wakeup `signal` fired on completion, and the slot the result lands in.
struct Job {
    address: String,
    signal: Signal,
    slot: Slot,
...
impl Job {
    /// Mint a job and its host-facing [`Pending`] as a pair. They share one
    /// result `slot` and the two ends of one wakeup pipe — the worker keeps
    /// `signal`, the host keeps `ready`. The job is queued to a worker; the
    /// `Pending` stays with the caller to poll and drain.
    ...
    /// Deliver a finished lookup and retire the job: fill the result slot, then
    /// wake the host's poller. The order is load-bearing — the host drains the
    /// slot the moment the signal makes its `fd` fire, so the slot must be set
    /// first. Consumes the job; the worker is done with it once delivered.
...
impl Pending {
    /// Bundle the host's two handles for a queued lookup. Minted by [`Job::new`],
    /// which owns the shared `slot` and the pipe whose read end is `fd`.
...
impl OsResolver {
    /// Build a pool of `threads` workers draining a queue that holds at most
    /// `length` pending lookups. The pool caps both the threads and — since each
    /// queued job already holds a pipe — the file descriptors a flood of lookups
    /// can tie up: at most `threads + length` lookups are ever in flight, so
    /// [`start`](Self::start) sheds load (a retriable `WouldBlock`) rather than
        ...
        for _ in 0..threads {
            ...
            thread::spawn(move || {
                loop {
                    ...
                    // The blocking `getaddrinfo`, off the scheduler thread. Each
                    // blob is the canonical "ip:port" string — debuggable, and
                    // `socket` recovers the family from it.
    ...
    pub(crate) fn start(&self, address: String) -> Result<Option<Pending>, Status> {
        ...
        match self.sender.try_send(job) {
            ...
            // end (in the dropped `pending`) closes with it; report the shed so
