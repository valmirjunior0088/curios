...
Runtime-only Curios engine (lib) + the launcher stub (bin `curios-rt`). Deserializes a precompiled module and runs it on wasmtime, wiring the `sys.io_*` host imports (plus any `env.*` bindings an embedder supplies for a program's own `foreign` declarations); **never** links Cranelift or Binaryen.
