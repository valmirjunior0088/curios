...
//!
//! The boundary validates what it has not already seen validated, and projects what is not already projected — which for the replay entrypoint is the entry's own items and the registry entries it adds. The prelude arrives immutable and checked from `curios-prelude`'s restore, so validating and projecting it again is a walk of the whole standard library for an answer already in hand. Doing both was measured at ~320 ms of a ~1000 ms release compilation of a one-line program, and it fell inside the erasure context's step budget, which a debug build then exceeded on `programs/hello_world.crs`.
