...
pub(crate) fn convert_intrinsic(
    ...
    let (this, that) = match !(literal(&this) || literal(&that)) && (stuck(&this) || stuck(&that)) {
        ...
        true => {
            let this = Nat::normalize(context, Term::intrinsic(this.clone()))?;
            let that = Nat::normalize(context, Term::intrinsic(that.clone()))?;
            let this = int_normalize(context, this)?;
            let that = int_normalize(context, that)?;
    ...
    // **Two `Bool` terms, one of them a connective, are first put to the truth table over their atoms** — the kernel's rule, stated once more here: it decides what no leaf set or local law relates, De Morgan and absorption and distribution, and changes no spelling. A metavariable among the leaves is an atom like any other, since agreement at every assignment holds whatever it is solved to. Undecided is not unequal, so everything below runs as it did.
    if decide_bool(
        context,
    ...
    let (this, that) = match align_comparisons(context, &this, &that)? {
        Some(pair) => pair,
        None => (this, that),
    };
    ...
    if !Convert::compare_levels(context, this.result_universes(), that.result_universes())? {
        return Ok(false);
    }
