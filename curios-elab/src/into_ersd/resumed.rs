...
//!
//! Erasure of a compilation unit resumes over what earlier erasures established, and that scope has *two* halves which must describe the same thing. The [`ErasedUnit`] is the arena the builder resumes over and the environment mapping names to operands; the [`Module`]s beside it are the Core those operands were erased from, re-seeded so the unit's own re-derived types reduce through them.
//!
...
use {
    super::{ErasedUnit, lower::UniverseErased},
    curios_core::Module,
};
...
impl<'a> Resumed<'a> {
    ...
    pub fn of(cores: &'a [&'a Module], arena: ErasedUnit) -> Self {
        Self { cores, arena }
    }
    ...
    pub(super) fn into_arena(self) -> ErasedUnit {
        self.arena
    }
}
