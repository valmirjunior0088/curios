...
fn copy_pure(prim: &PurePrim) -> PurePrim {
    ...
    match prim {
        ...
        BinGet(a, b) => copy!(BinGet, a, b),
        BinSlice(a, b, c) => copy!(BinSlice, a, b, c),
        BinAppend(a, b) => copy!(BinAppend, a, b),
        BinConcat(operands) => BinConcat(operands.iter().map(deep_copy).collect()),
