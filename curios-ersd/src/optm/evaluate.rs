...
//! `curios-cont/src/optm/scalar_eval.rs`, the wasm-faithful table shared by
...
fn plan(module: &Module) -> Vec<Rewrite> {
    // `to_cont` requires a definition to precede its uses, and item order is
...
fn apply(module: &mut Module, rewrites: Vec<Rewrite>) {
    ...
    // capture lists predate the rewrite. `to_cont` threads every global
...
//
// A row-by-row transcription of `curios-cont/src/optm/scalar_eval.rs` for
