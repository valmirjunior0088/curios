...
///
/// **What this pins, and what it does not.** Mutation-checked by making `free_values_settled` answer `true`: the candidate then folds and a copy of `inner` appears, which is what the count asserts. The module still verifies under that mutation, because here the group is spliced into `host`'s own block and `z` is in scope there — so this is a pin on the *decision*, not a reproduction of the miscompilation. The dangling reference needs the copy to travel, which takes a later round reusing it from another block; `curios`'s `a_deadlock_reports_how_many_fibers_wait_on_unsatisfied_conditions` is what reaches that end to end, through `/std/Async`'s selection over no offers.
#[test]
