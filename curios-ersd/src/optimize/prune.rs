use crate::{Item, Module, optimize::call_graph::CallGraph};
...
/// full on every compile. But `into_cont` lowers *every* item into `main`'s entry
...
/// lost once `into_cont` turns each item into anonymous CPS blocks — which is why
/// the prune lives here rather than in `cont::optimize`, where the dead initialization is
