...
use {
    super::CallGraph,
    crate::ersd::{Apply, Argument, Func, Item, Module, Name, Prim, PurePrim, Rec, Subterm, Term},
    cursor::SliceCursor,
...
mod tests {
    use {
        super::*,
        crate::ersd::{CellPrim, Match, NatMatch},
    };
    ...
    fn index_only_buffer_walk_threads_a_cursor() {
        ...
        let step = Subterm::Let(crate::ersd::Let {
            name: "h".to_owned(),
    ...
    fn suffixes_keeps_the_copy() {
        ...
        // the copy stays.
        let step = Subterm::Tuple(crate::ersd::Tuple {
            fields: vec![
                name_term("b"),
                call(
                    "suffixes",
