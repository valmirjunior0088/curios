//! Blocks, statements, terminators, right-hand sides, functions, and
//! recursive groups — the direct-style ANF structure of the representation.
//!
//! Every non-atomic computation is bound by a `Let` statement, so evaluation
//! order is exactly statement order — the operand law's home, and the reason
//! effect ordering is a structural property rather than per-pass discipline.
//! Branch and fold bodies are blocks referenced by [`BlockId`]; every block has
//! exactly one structural owner and block references are acyclic. Recursion
//! happens only through function and recursive-group identities, never by
...
pub enum Rhs {
    ...
    /// constructor's payload (positionally, in payload order) in its block;
    /// `default` covers every constructor without an arm. The arms plus the
    ...
    /// Dispatch on a `Nat` scrutinee by literal key, falling through to
    ...
    /// `Nat` induction as a bounded fold: `zero` computes the base result, and
    /// `step` computes each successor result from the predecessor index and
    ...
    /// Sequence elimination as a right fold: `empty` computes the base result,
    /// and `step` sees the current element, the suffix view after it, and the
    /// accumulated result of folding that suffix. A first-class loop form —
    ...
    /// A mutable-cell operation (see [`CellOperation`]). A cell operation's
    /// identity is its program point: it is never deleted for an unused
...
/// One arm of a [`Rhs::MatchVariant`]: the constructor it matches, the value
...
pub enum Statement {
    ...
    /// Introduce a group of mutually recursive functions, all in scope in each
    /// other's bodies and in the rest of the enclosing scope. A nonrecursive
    /// function binding is a one-member group.
    Functions { functions: Vec<FunctionId> },
    /// Introduce a mutually recursive group mixing functions and eagerly
    /// computed values (see [`RecGroup`]).
...
/// A function: its ordered runtime parameters and its body block. No capture
...
/// A mutually recursive group mixing functions and eagerly computed values.
/// Every member is in scope in every member body and initializer and in the
/// rest of the enclosing scope. Member order is source order; initializers run
