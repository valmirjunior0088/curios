...
The curios erased IR (post type-erasure): ersd→ersd optimization (`optm/` — prune, the `worker_wrapper` engine over a shared `call_graph`/suffix-view cursor) and lowering to continuation-passing style (`to_cont/`).
