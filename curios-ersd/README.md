...
The Curios erased IR (post type-erasure): ersd→ersd optimization (`optimize/` — prune, the `evaluate`/`specialize` compile-time staging pair, and the `worker_wrapper` engine over a shared `call_graph`/suffix-view cursor) and lowering to continuation-passing style (`into_cont/`).
