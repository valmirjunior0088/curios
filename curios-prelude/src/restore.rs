...
///
/// There is deliberately no content-digest check. The archive reaches this function through `include_bytes!`, so it is a constant in the executable's own read-only data, and the digest `build.rs` exported alongside it is another constant from the same build: hashing one to compare it against the other cannot fail except under corruption of our own image, and covers only the prelude rather than any of the compiler's code. It could not detect a stale or substituted archive, because there is no runtime file to be stale or substituted. Bytecheck below is the part that does real work — it is what makes handing out references into these bytes sound — and the schema and fingerprint reject an image from any other build.
...
///
/// The universe invariants are *not* re-checked here. They are asserted once by `build.rs`, on the value it is about to serialize, and [`validate_bytes`] establishes that these bytes are exactly the bytes written from that value: the content digest pins them, bytecheck confirms the archived graph is structurally sound, and the schema and source fingerprint reject an image from any other build. Walking the whole standard library again per compilation to re-derive an answer already settled cost ~175 ms of a ~680 ms release compile of a one-line program.
