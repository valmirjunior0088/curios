...
**Decision.** `/syn` owns exactly the names Rust lowering emits — the proof-certified literal structures (`/syn/Char`, `/syn/Str`) and the concepts syntax dispatches through, declared at `syn.crs` top level. Operator witnesses live in the `/std` operator facades and other witnesses beside their types; the canonical Rust registry of the hidden lowering targets is this crate's `src/syntax.rs`, and the registry contract belongs to `curios-text`.
