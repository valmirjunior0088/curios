...
pub(super) fn printed(module: &cont::Module) -> String {
...
pub(super) fn i32_result(module: &cont::Module) -> i32 {
...
pub(super) fn int_result(module: &cont::Module) -> i32 {
...
pub(super) fn f32_result(module: &cont::Module) -> f32 {
    let (system, io) = MockHost::builder().build();
    crate::run_wasm(&to_wasm(module), system, ForeignBindings::empty()).expect("run failed");
    let bytes = io.output();
...
pub(super) fn traps(module: &cont::Module) -> bool {
    let (system, _io) = MockHost::builder().build();
