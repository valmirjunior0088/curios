...
// not constant-folded away, and prints through `Io/print(Nat/to_str(...))` to keep
// the kernel live.
...
const TREES: &str = r#"
    ...
    Io/print(Nat/to_str(sum(build(d, 1))))
    "#;
...
const MUTUAL_RECURSION: &str = r#"
    ...
    let n : Nat = Lst/len(proc/args());
    let start = n <= 0;
    Io/print(Nat/to_str(match start : Nat | true => ping(n) | false => pong(n) end))
    "#;
