...
///
/// **One negative result, worth as much as the positive ones.** Typing `List` slots at the rope base was meant to bite into the `is_subtype` libcall class, and it did nothing: `ref.cast (ref $rope/N)` is 51 in five programs and 72 in `spines` in *both* arms. Reading where those casts actually sit says why, and corrects the census's framing of them. Only **39** of `spines`' 72 target the non-final base `$rope/list` and are therefore libcalls; the other 33 target `(sub final $rope/list …)` subtypes and short-circuit to inline compares. The 39 are spread through ordinary code — **12 in `/std/Map/insert1`**, 10 in `main`, the rest in the UTF-8 walks and `io/bind` — not in the box helpers, and not in fields. Exactly two locals in the whole module are declared at a rope type, which is the cause: a value the analysis already knows is a list is held at the top reference type, so every use re-casts to a non-final base. `Table::local_type` states the gap outright — its reference arms are unreachable because `Storage` only ever answers a scalar carrier. The work that would move this class is a local declared at the reference carrier its uses agree on, which is this campaign's idea applied to locals rather than to heap fields.
///
/// # Nominal rows, 2026-08-20
///
/// Four commits: the family vocabulary generalized to a *nominal row* (a family is a row whose slot zero is a tag, a product a row without one), product schemas keyed by schema rather than by arity, closure slots declared at `(ref null $envr/N)`, and a family-typed reference slot admitted exactly where the row does not widen. Same method as above, `before` = the typed-slots commit.
///
/// **The result is flat, and the reason is worth more than the figure.** Six of eight workloads agreed between passes inside ±1.5%. `spines` did not — one pass read −7.7% and the next +5.1%, with the *baseline* itself moving 65 to 59 ms — so it was retaken alone at 25 interleaved reps: **min 56.4 to 57.8 ms (+2.4%), median 60.9 to 63.4 (+4.2%), p10 identical at 58.8**. Flat to marginally worse, and nothing like the −7.7% the contaminated pass suggested. The first run was taken at a one-minute load of 3.15 rather than the sub-1.0 the earlier figures required, which is exactly what the two-pass rule exists to catch.
///
/// **Why flat: the corpus has no surviving product allocation to improve.** In the *unoptimized* wasm — before Binaryen sees it — each product row appears exactly once, as its own type declaration, and `struct.new` on one appears **zero** times. Curios' own passes (constructor specialization, parameter splitting, inlining) already scalarize every product in these programs; `/syn/Str/Scan`, `/std/Map/Map`, `/syn/Eql` and `/syn/Cmp` are declared and never built. The rows that survive to the shipped module are variant families alone — `Option`, `Result`, `Handle/Error`, `Map/Node`, `Chain` — and those the typed-slots commit had already typed. The box/unbox class moves by 2–3 sites out of ~300 accordingly.
///
/// So this batch buys no time on this corpus and was not expected to once the static counts were read. What it does buy is a representation that stops lying — `CpsSlot::Product(width)` would have been *wrong* the moment products were keyed, since a product value stops being a `$tuple/N` — one vocabulary where there would otherwise have been three, and a rule that types a long-lived product's fields in code this corpus does not contain. The measurement is recorded because a null result that explains itself is what stops the next campaign re-deriving it.
///
