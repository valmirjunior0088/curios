...
fn accumulation_loops_are_linear_by_construction() {
    ...
    //
    // **`head_of` keeps this a runtime measurement, and that is now its whole job.** `Bytes/slice` states `10 <= Bytes/len(b)`, a *decided* proposition, so its subject stands in a type and the obligation is discharged by reducing that subject. Written directly as `Bytes/slice(built, 0, 10)`, the compiler evaluates `go(100000, x[])` at the type level — about seventeen million reduction steps, sixteen times the default budget — and refuses. Behind `head_of` the subject is a parameter: `b` is opaque, the guard refines it once and generically, and the call site passes an ordinary value, so nothing is computed at elaboration time and what this test times is the emitted program.
    //
    // **It used to work around something far worse, and the change is worth knowing.** Fusing an all-literal concatenation recopied the whole accumulator every step, so the direct spelling spent a *quadratic* volume of construction against a linear step count: it did not refuse or time out, it exhausted the machine. `curios-core`'s `FUSION_CAP` and its measure removed that, and the direct spelling is now an ordinary bounded computation that simply asks for more budget than the default allows. `tests::numeric`'s `a_bound_on_a_small_computed_subject_discharges` writes it directly at a size the default admits, which is the spelling a user actually reaches for; `curios`'s `tests::reduction` holds the figures for both halves.
    //
