...
//
// Two occurrences of one syntactic term then denote two different values. After `Cell/set(c, false)` the inner `match Cell/get(c)` refines that same term to `false`, `p` re-reads at `Eq(false, true)`, and `/std/Bool/false_neq_true` turns it into `/std/Bool/False`. The arm deriving it is *reachable*: the first read is `true`, so the outer arm runs, and the second is `false`, so the inner one runs too.
//
