...
fn matrix_match_rejects_a_motive_on_tuple_head() {
    // A tuple-headed matrix explodes into projections and builds no core
    // `Match` node, so there is no eliminator for a motive to be checked
    // against. Rejected rather than silently discarded.
    let source = r#"
        use /std/{Nat, Handle};
        let f(p : { Nat, Nat }) -> Nat =
            match p : (q) => Nat
...
// === Motives ================================================================
//
// A motive is a term checked against the eliminator's motive type,
// `(ī : Ī(p̄)) -> I(p̄, ī) -> Sort`. There is no motive grammar: what follows
// `:` is parsed by `parse_term` and checked like any other term.

// The motive need not be a lambda at all. A top-level family of the right type
// is eta-expanded into the motive scope, so an elimination can name the family
// it proves rather than restating it.
#[test]
...
// A motive that ignores every binder is written with `_`s, one per index and
// one for the scrutinee — a constant motive is a lambda like any other, not a
// separate rung.
#[test]
...
// A motive binder's annotation is an ordinary type in an ordinary position, so
// the scrutinee binder's annotation may name the index binders written before
// it — recovering the eliminated family on the motive line. This is the
// dependent-lambda-telescope rule (`tests::binders`) applied to a motive.
#[test]
...
// Plicity is expressible because the annotation is a real application: `Eq`
// hides its type parameter, so `Eq(s, t)` is how it is written here, and the
// old flat slot list that spelled it `Eq(A, s, t)` has no counterpart.
#[test]
...
// A `| _ =>` catch-all on an indexed family. Every motive binds its indices
// whether or not the body uses them, so a default no longer collides with a
// "pattern motive": the enumerated arms are checked at their own case target
// indices and the default at the scrutinee's actual ones.
#[test]
...
// The binder count is checked against the index telescope, not inferred, so an
// under-bound motive reports as itself instead of as a domain mismatch.
#[test]
