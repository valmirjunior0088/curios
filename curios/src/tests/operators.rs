...
fn bool_logic_and_of_str() {
    assert_eq!(
        ...
        let _ = Io/write(Io/stdout, Str/to_bytes(Str/concat(Bool/to_str(computed), Bool/to_str(parsed))))!;
        /std/Io/pure(())
        "#),
        b"truefalse"
    );
...
fn infix_add_on_a_user_record_resolves_its_witness() {
    assert_eq!(
        run(r#"
            ...
            let q : Point = p + p;
            let _ = Io/write(Io/stdout, Str/to_bytes(Nat/to_str(q.y)))!;
            /std/Io/pure(())
        "#),
        b"8"
    );
...
fn infix_literal_against_a_user_type_is_rejected() {
    let source = r#"
        ...
        let q : Point = p + 1;
        let _ = Io/write(Io/stdout, Str/to_bytes(Nat/to_str(q.x)))!;
        /std/Io/pure(())
    "#;
    error(source);
}
