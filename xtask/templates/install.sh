...
case "$os $arch" in
    ...
    *) die "no prebuilt binary for $os $arch; build from source with: make curios" ;;
esac
...
version=$("$work/curios" --version 2> /dev/null) ||
    die "the downloaded binary does not run on this system (usually an older glibc than the release was built against); build from source with: make curios"
