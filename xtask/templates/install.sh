...
case "$os $arch" in
    ...
    *) die "no prebuilt binary for $os $arch; build from source with: git clone $REPO && cd curios && cargo x build" ;;
esac

if command -v curl > /dev/null 2>&1; then
    fetch() { curl -fsSL "$1" -o "$2"; }
    fetch_with_progress() { curl -fL --progress-bar "$1" -o "$2"; }
elif command -v wget > /dev/null 2>&1; then
    fetch() { wget -q "$1" -O "$2"; }
    # `--show-progress` arrived in GNU wget 1.16, and without it the meter cannot be separated from the rest of the chatter `-q` suppresses. An older wget downloads silently rather than verbosely.
    if wget --help 2>&1 | grep -q -- --show-progress; then
        fetch_with_progress() { wget -q --show-progress "$1" -O "$2"; }
    else
        fetch_with_progress() { fetch "$1" "$2"; }
    fi
else
    die "neither curl nor wget is available"
fi

# A meter is for someone watching one. Piped into a CI log it is a line of redrawn noise, and curl draws it there regardless of the pipe, so whether to ask for one is decided here rather than left to the tool.
[ -t 2 ] || fetch_with_progress() { fetch "$1" "$2"; }
...
fetch_with_progress "$base/$asset" "$work/curios" || die "could not download $base/$asset"
...
version=$("$work/curios" --version 2> /dev/null) ||
    die "the downloaded binary does not run on this system (usually an older glibc than the release was built against); build from source with: git clone $REPO && cd curios && cargo x build"
