...
//!
//! **A compilation is units folded over a dependency order, and this crate is the fold.** [`compile_units`] takes a [`Scope`](curios_unit::Scope) — every unit already compiled, borrowed in order — and a slice of [`UnitSource`](curios_text::UnitSource)s, and runs each through *lower → elaborate → judge → erase* against the base and everything before it. [`compile_entrypoint`] is that fold's last step, for the one unit that carries an entrypoint and owns the empty prefix.
//!
