...
//!
//! **This crate does not know which unit is the standard library.** It has no `curios-prelude` dependency outside `[dev-dependencies]`, where its own fixtures need one because a program needs a prelude to compile against. A product names its standard library — `curios` and `curios-js` restore the archive and pass it as the first unit in scope — exactly as the `/syn` registry is already the caller's to supply. Everything wasm-native — Binaryen, Cranelift precompilation, execution — lives downstream in `curios`/`curios-runtime`: this crate stops at the wasm module plus the program's harvested `ForeignStore`.
