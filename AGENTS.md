...
- **`README.md`** — project pitch, install via `cargo install`, CLI subcommands (`run`, `check`, `compile`) with `--timeout`, `--print`, and `compile --output-path`, minimal `/sys/Io/write` entrypoint.
- **`SYNTAX.md`** — complete language reference: lexical basics, top-level forms (`let`, `rec`, `union`, `mod`, `use`), every term and type form including holes and `with`/bang sequencing, literals, canonical idioms for sum and recursive types, and an appendix of `/sys` operation tables (`Nat`, `Int`, `Flt`, `Bin`, `Arr`, `Bln`, `Io`).
