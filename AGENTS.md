...
- Names are path-qualified with `/`: `Option/none`, `/std/Arr`, `/syn/Str`; a leading `/` is absolute.
...
- **Concepts / witnesses / instance arguments** are the ad-hoc-polymorphism layer (see [SYNTAX.md](SYNTAX.md#concepts-witnesses-and-instance-arguments) for the surface syntax). The Rust-side implementation, file by file:
  ...
  - `Show`/`Ord` live in `curios-text/std/`; `Eql` is homed in `/sys` (operator dispatch) and `Monad` in `/syn` (the postfix `!` desugars each site to `/syn/Monad/bind(action, continuation)` in `to_core/lower.rs` — every value body is a region root, there is no `let !` header, and the action's type resolves the witness), each with a `/std` facade.
...
- **The `curios-js` wasm32 build is gated by the `wasm` job in `check.yml`** (`wasm-pack build curios-js --release --target web`, same command as `release.yml`'s `build-wasm`). The host-target done bar does not cover it — `cargo check --workspace` type-checks `curios-js` for the host, where the wasm-bindgen externs are panicking stubs — so when touching `curios-js` or its dependencies, run the wasm-pack build locally too rather than discovering the breakage in CI.
