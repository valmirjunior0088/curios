...
`curios-binaryen/build.rs` fetches Binaryen's source release and builds it into a static lib via CMake, linked via FFI from `curios-binaryen/src/` to optimize emitted wasm. The wasmtime engine + host stack live in `curios-rt/src/` (`run_bytes` deserializes a `.cwasm` and runs it; `instantiate` wires the `sys.io_*` host imports and, via the required `ForeignBindings` argument, any `env.*` bindings an embedder supplies for the program's own `foreign` declarations).
...
- **Concepts / witnesses / instance arguments** are the ad-hoc-polymorphism layer (see [SYNTAX.md](SYNTAX.md#concepts-witnesses-and-instance-arguments) for the surface syntax). The Rust-side implementation, file by file:
  ...
  - Every infix operator except `&&`/`||` dispatches through the `/syn` operator concepts (`Add`/`Sub`/`Mul`/`Div`/`Rem`/`Eql`/`Cmp`, declared in `curios-text/syn.crs` with their primitive witnesses in the `/std` operator facades; the operator→concept table lives in `curios-core/src/syn_names.rs`, the rewrite in `elaborate_infix`) — there is no separate primitive-operator path, and codegen parity with the bare prims is pinned by `curios/src/tests/codegen/parity.rs`.
