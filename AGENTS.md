...
- **`SYNTAX.md`** — complete language reference: lexical basics, top-level forms (`let`, `rec`, `union`, `mod`, `use`), every term and type form including holes and `let !`/bang sequencing, literals, canonical idioms for sum and recursive types, and an appendix of `/sys` operation tables (`Nat`, `Int`, `Flt`, `Bin`, `Arr`, `Bln`, `Io`).
