...
- **The `curios-js` wasm32 build isn't gated on every PR.** `wasm-pack build curios-js --release --target web` only runs in `release.yml` (triggered by a `release/*` tag), not in `check.yml`. A change to `curios-pipeline`, `curios-js`, or their dependencies can break the browser build silently until a release is cut — if you touch any of those, build it by hand before considering the change done.
