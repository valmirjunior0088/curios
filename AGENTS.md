...
- A publicly exposed signature may not mention a private item (`PrivateItemInPublicInterface`). The post-resolution audit follows re-exports and straightforward transparent aliases; parameter/index dependencies are always interface, while fields and constructor signatures are checked only where the representation is exposed. Ordinary definition bodies remain exempt.
