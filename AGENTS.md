...
- A `pub` item's declared signature may not mention a private item (`PrivateItemInPublicInterface`, checked in `into_core` via `Context::check_public_interface` — signatures only, bodies exempt; a `struct`'s hidden field types are not interface).
