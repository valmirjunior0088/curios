...
impl UniverseContext {
    ...
    ///
    /// **A method on the data, not a judgment.** Both checkers used to spell this out, character for character, a second opinion known to be worth nothing (see `documentation/soundness/whole-module-passes/validate_universes-inside-zonk_module.md`). The predicate is too simple to have two genuine implementations — any "independent" rewrite would agree by construction rather than by independence — so the copies bought a diff test over a copy of themselves. It lives here for the reason [`Level::structurally_leq`] does: it decides a property of the representation by looking at it, runs no solver, and admits nothing on its own. Deciding *satisfiability* is the opposite case and stays written twice — see `curios-analysis`'s `satisfiable` for why.
