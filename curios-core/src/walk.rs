...
//!
//! **This driver keeps its explicit stack, deliberately and indefinitely.** Its internals do show the tells of a frame machine — `Frame::Exit(Term, usize)` is a recursive frame's locals written out, `exit` runs on pop carrying `&mut S`, and the `checked_sub(child_count)` offset is a calling convention by hand — and a recursive reimplementation behind this same API would leave the seam, the `any_child_term` fold and every call site untouched. It is not done because the stack is not what this driver is *for*: it exists to enumerate children in one place and to give each analysis somewhere to hang its own memo, and memoization is what the measurements behind it actually cured. `curios-elab`'s `totality::reach` records both — a quadratic in *paths* through a shared DAG, cured by deduplicating on node identity in the `enter` hook, and separately a depth overflow. Only the second would change here, in a component that decides nothing, has no second implementation to be read against, and is reached by the kernel's erasure obligations, both positivity and totality drivers, and the per-node memo fill behind every `free_vars()` call. Reopening it wants a measurement, not the criterion.
//!
