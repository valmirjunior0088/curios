...
///
/// **What remains is the elaborator's, and it is a residue with a different source.** Its retention still grows quadratically in a literal's length — about three-quarters of its pre-machine figure, saturating this quota near five thousand characters — so the chain explained at most a quarter of it, and the remainder is unattributed. What the same probe shows is that saturating costs the literal nothing: the refused entries were not ones the walk re-needed, and the per-character units stay linear well past the saturation point. The open question is the source, not the ceiling.
