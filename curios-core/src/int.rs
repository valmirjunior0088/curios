//! The signed carrier's sum normal form: a literal constant over a linear combination of monomials with integer coefficients — [`Nat`](crate::Nat)'s form over ℤ, where a subtraction is a negative coefficient rather than a stuck node and a negation is the coefficient `-1`.
//!
