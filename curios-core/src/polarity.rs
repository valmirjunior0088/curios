...
//!
//! Representation, not analysis. The vector this describes is stored on `InductDecl`/`StructDecl` and rides the prelude archive, so the type and its lattice operations belong with the registry entries that carry them, while the occurrence relation that *computes* them stays in `positivity`.

use std::fmt;

/// How a type former uses one parameter, or at what polarity one declaration occurs inside another.
///
...
impl Polarity {
    /// Whether an occurrence at this polarity is admissible in a recursive position — the whole acceptance test, applied to one entry.
    ...
    /// Sign multiplication: a target occurring at `self` inside an argument whose former uses that argument at `outer` occurs at `outer ∘ self`.
    ///
