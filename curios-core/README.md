...
Sits between [`curios-text`](../curios-text) (which lowers surface syntax into core via `text/into_core`) and [`curios-ersd`](../curios-ersd) (which it erases into via `core/erase`). Code dependencies run opposite to data flow: this crate depends on `curios-ersd`, not the other way around.
