...
fn print_tail<'a>(tail: &'a Tail) -> Printer<'a> {
    match tail {
        ...
        Tail::Host(host) => match host {
            ...
            HostTarget::IoSocket { addr, resume } => flat([
                pure("Io.socket "),
                print_value_name(addr),
                pure(" "),
                print_block_name(resume),
            ]),
            HostTarget::IoBind {
            ...
            HostTarget::IoSetNonblocking { handle, on, resume } => flat([
            ...
            HostTarget::IoSetRecvTimeout { handle, ms, resume } => flat([
            ...
            HostTarget::IoSetSendTimeout { handle, ms, resume } => flat([
            ...
            HostTarget::IoSetReuseaddr { handle, on, resume } => flat([
