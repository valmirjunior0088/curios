//! The foreign-function store — the generic description of a host call, from
//! which each consumer derives its own view of the boundary.
//!
//! A [`ForeignFunction`] is one host call, and it is self-describing: its
//! `namespace`/`name` pair is the wasm import (`sys` and a fixed name for a
//! builtin, `ffi` and the declaration's fully qualified name for a user's own
//! `foreign` declaration — the wire-level ABI contract between the emitter and
//! the runtime linker), and its [`WireSignature`] names the operands and
//! results and gives each a [`WireType`]. Every host call is effectful, so
//! reducing one at the type level is always an error — the effect cannot
//! happen at compile time. The IR nodes carry the function as an `Arc`, so
//! every stage reads what it needs straight off the node instead of keeping an
//! independently hand-written spelling in lockstep:
//!
//! - the `/sys` prelude declaration, or a user's own `foreign` declaration
...
//!
//! A [`ForeignStore`] is the set of foreign functions declared under one tier.
//! [`host_ops`](super::host_ops) seeds the fixed builtin (`sys`) tier,
//! consumable only by the standard library, created per compilation by the
//! pipeline driver; a second store, accumulated from a program's own `foreign`
//! declarations (`curios_text`'s generated foreign signature), holds the `ffi`
//! tier. The two are never merged, but the wasm namespace is the row's own
//! `namespace` field, stamped at declaration time — the store split only
//! governs who may consume a tier. `exit` is deliberately absent from either
//! store: it traps rather than returns and its guest type is the polymorphic
//! bottom `(@A : Type) -> Nat -> A`, which a first-order [`WireSignature`]
...
/// `Handle` rides the same wire shape as `Bin` (a handle is its token bytes)
/// but stays a distinct guest type.
...
/// unit tests snapshot the builtin set). `namespace` is `sys` for a builtin
...
/// order the prelude surfaces them under `/sys`. Rows are `Arc`ed so the IR
