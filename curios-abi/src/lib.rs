...
//!
//! Both ends cite these definitions: `curios-runtime`'s `host` module when it lowers a `Status`/`Poll`/`Mode` to the wire and when it types the `ffi.*` imports, and the compiler when it mints the `/sys/Handle` prelude declarations, checks host-op operands, and emits the wasm imports. This crate sits below every compiler stage and both runtimes — its only dependencies are the shared foundations `curios-num` and `curios-archive` — so the front-end and the runtime both import it without inverting the pipeline's layering. Why the builtin list is authored once and projected, why `exit` is a diverging row, why the wire vocabulary nests no list, and why a builtin is an identity are `README.md`'s decisions.
