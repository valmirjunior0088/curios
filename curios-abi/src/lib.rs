...
//!
//! Both ends cite these definitions: `curios-runtime`'s `host` module when it lowers a `Status`/`Poll`/`Mode` to the wire and when it types the `ffi.*` imports, and the compiler when it mints the `/sys/Handle` prelude declarations, checks host-op operands, and emits the wasm imports. Because the two sides derive from the same definitions, they cannot drift. This crate is a leaf — it depends on nothing, so both the front-end and the runtime can import it without inverting the pipeline's layering.
