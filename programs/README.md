...
- `make curios/profile CURIOS_PROFILE_SOURCE=programs/<file>.crs` — one compilation under the tracing profiler.
...
**Samples.** `hello_world.crs` — also `make curios/profile`'s default subject — and `dependent_vectors.crs`, which show the language rather than measure it.
