...
The wire types are `Nat`, `Int`, `Bool`, `Flt`, `Bytes`, `Bits`, `Handle`, and `List(T)`, spelled bare: the wire grammar is a closed vocabulary that resolves no names, so `/std/Nat` is refused where `Nat` is meant. Eight words that look like types and are not. A `Nat` or `Int` crosses as a 32-bit word although the program's are unbounded: a `Nat` argument crosses below `2³¹` and an `Int` between `-2³¹` and `2³¹ - 1`, and a larger one stops the program rather than crossing changed. A result comes back as a 32-bit word too — a `Nat` read unsigned and an `Int` signed — and the program boxes it, so every result the word holds arrives whole. A wire signature is a wire result for a zero-argument foreign, or a parenthesized wire parameter list followed by `->` and a wire result.
...
`List` does not nest, and its element vocabulary is narrower still: an element must be `Nat`, `Int`, `Bool`, `Bytes`, `Bits`, or `Handle`, so `List(List(T))` and `List(Flt)` are both rejected. A list of `Nat`, `Int` or `Bool` crosses as one 32-bit word per element, each narrowed going out and boxed coming back by the program exactly as a lone one is, so an element past the word stops the program as an argument would; a list of `Bytes`, `Bits` or `Handle` crosses as its elements' payloads. A `Flt` crosses on its own as a raw binary64, and no operation has asked for a list of them. How the declaration reaches the embedder is the host ABI's concern rather than the surface language's.
