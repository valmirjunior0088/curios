...
- **Incrementality inside a unit and per-item recovery**, which are [A unit the store holds is a baseline](../design/toolchain/a-stored-unit-is-a-baseline-for-an-item-level-recompile.md)'s and are built; the loop over `/std` rests on them.
