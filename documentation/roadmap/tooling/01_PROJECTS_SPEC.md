...
- **M4-B3, a verdict is reused for the terms it was reached about** — landed, with its [SOUNDNESS.md](../../SOUNDNESS.md) entry, which is what makes it admissible rather than merely fast. Measured on a two-package project: **16.5 s cold, 2.1 s warm**. The key is the terms, the compiler binary's own content digest, and the ordered keys of the predecessors.
  - *Decided against this document, in the safe direction:* the certifier half is **the compiler binary's content**, not a fingerprint over `curios-cert` and `curios-analysis` sources. This document named that fingerprint's gap itself — a dependency bump changes what the certifier decides without touching those files — and a verdict surviving a change it should not have survived *admits*, which is the only soundness question of the two. A source fingerprint is also a maintained list, and the failure of forgetting an entry is silent. A content digest has no list. It is memoized against the binary's size and modification time, which is `sccache`'s arrangement, so the common path is one `stat`.
  - *Rejected on the way:* a per-build nonce (trivially sound, throws away every hit a reproducible rebuild should keep) and a version string (does not move while a compiler is being developed, which is the direction that admits — the arrangement GHC, Coq and Lean can afford because their users receive releases).
  - *Observed, not assumed:* the predecessor half of the key earns itself. A two-package project across two edits of its dependency stores **four** records, because the dependant's key moves when its dependency's does even though its own source never changed. That is the cumulative universe-seed table this document warned about, visible.
- **M4-B4, the erased artifact** — **nothing to do, and this document expected otherwise.** It anticipated per-unit erased artifacts that would each number from zero and need a relocation pass. They never did: `Scope::arena` returns *one* value, not one per unit, because each unit's erasure resumes over the previous one's — so a unit's erased half already **is** the prefix's artifact, cumulative from the prelude forward. Pair that with a key naming the exact ordered predecessors and the arena a restored unit carries always matches the prefix it lands in. B3's key bought B4; the "expect this half to move off the unit and onto the prefix" note on `curios_unit::Unit` was answering a question that had already been answered elsewhere.
- **M4-B6, what never caches** — **structural, not enforced.** Witness coherence has no verdict to cache: `Established::replay_definitions` rebuilds the table every elaboration from every module in scope, restored and freshly compiled alike, through the same `register_witness` with its orphan check and duplicate insert — *"the witness table is per-elaboration state and is not carried on a module"*. The visibility fixed point is the same shape, recomputed over the union of scope and unit. Neither needed a guard; both would have needed one if the table had been carried on a module, which is the design decision that made this free.
  - *Decided each rather than the group, as asked:* strict positivity, declaration sizing and concept-registry validation are per-unit properties stable under extension, mounts are disjoint and units ordered, so nothing later can add a constructor to an earlier unit's inductive. All three cache with the unit, and they agree by having the same reason rather than by being lumped.
