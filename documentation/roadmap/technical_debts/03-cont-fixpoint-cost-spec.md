...
Instrumented, located, and the first cure landed: `split_parameters`, `eliminate_dead_parameters` and `contify_calls` each drain every candidate one snapshot admits, and the headline program's fixpoint went from 57 rounds and 2 858 ms to **11 rounds and 933 ms**. `curios_cont::optimize` names and times every pass and records whether it fired, and `curios`' `fixpoint_pass_measurements` carries the before and the after. The first revision of this document carried three hypotheses it could not separate; the measurement separated them, the drains answered the one that held, and what remains is inside two passes rather than between them.

## Why it exists

`curios_cont::optimize` was almost the whole of an ordinary `Toml/decode` compile — **97.6%**, 4 719 ms of 4 836 ms, when first measured on `aarch64-apple-darwin` over a program nothing named; **89%**, 2 858 ms of 3 196 ms, re-taken on `x86_64-unknown-linux-gnu` over the census's `TOML_DRIVER`, which is now the program the probe names. After the drains it is **74%**, 933 ms of 1 259 ms, on the same host and program. Every other stage is noise beside it: elaboration, the kernel, erasure and the lowering into Cont are each under 25 ms, wasm emission is 115 ms, and the whole erased-arena optimizer with its verifier is about 75 ms.
...
- **`into_wasm` is not negligible on this host and is not the problem.** 115 ms against the fixpoint's 933 ms — now 9% of the compile — where the first measurement had it at 39 ms. Worth a read once the fixpoint is out of the way.

What is left stands inside two passes rather than between them. `forward_aggregate_projections` is now the largest line, 304 ms with **225 ms in the first round**: it forwards the door's 1 459 projections — and now every split's, since the splits land at once — by rescanning the module and rebuilding its construction map once per projection forwarded. `inline_known_calls` is second at 194 ms, walking three bodies per candidate call site per sweep. A sweep's own per-candidate walks are the same shape one level down, and the split sweep already paid for one: on the sixteen-rule grammar its first round cost 285 ms and 4.0 M allocations while every split re-scanned the module for its carriers, and 167 ms and 0.3 M once those were indexed per sweep. `fold_intrinsic_identities` has the same find-one-and-rescan shape as the first and, on this program, nothing to find.
...
**Next: the two find-one-and-rescan passes become sweeps.** `forward_aggregate_projections` and `fold_intrinsic_identities` collect every forwardable projection against one construction snapshot, collapse the substitution chains the way `known_values` does, rewrite in one walk, and splice the dead nodes out in one pass — the shape `eliminate_dead_bindings` already has. This is the first round's 225 ms, and it grew with the drains because the splits it cleans up after now land together.

**Then `inline_known_calls`' per-candidate body walks**, which are three per call site per sweep and are now the second line.
