...
| `BigNat/succ.crs:56`, `:61`, `:94`, `:100` | 16 | packed constant folding versus cons/append/concat decomposition | Item 4 — dropped 2026-08-08 |
...
| `Str/utf8.crs:180` | 1 | investigation closed 2026-08-08: the arm's expected type is *inherently* refinement-dependent — it spells `x[]` where `nz`'s type spells `b`, so its only spellable solutions are well-typed exclusively under the arm's refinement, which the refinement-free solution discipline rightly refuses (a solution can travel outside the arm through another metavariable, where the kernel would reject it) | resolved by deleting `/syn/False/absurd` from the library: the zero-arm match is the primitive doing the same job without a metavariable — and covering this refinement-dependent case `absurd` could not — the compiler never emitted the name (a `/syn`-law violation), and all 48 corpus call sites respelled as `match proof end` |
...
| Pruning | **Resolved without implementation 2026-08-08.** The re-measurement its section demanded came back empty: every named consumer landed with the defect-ledger materialization fix, the acceptance probe — `Future/new` re-declared with an implicit parameter, callers dropping the argument — passed against the unmodified guard, and no corpus site demands restriction-commits. The embedded-metavariable postponement stays as the designed behavior, now honestly reported by Item 1's diagnostics | Real pruning is dropped, η-heads-style: reinstate when a real program blocks on an equation solvable only by restriction, not merely postponed by it |
| Packed views | **Landed 2026-08-08**, as `packed_literal_view` in `curios-elab/src/convert/intrinsic.rs`: a `Bin` literal opposite an `append`/`concat` spine of the same grain decomposes length-directedly (an `append` takes the last atom, a `concat` splits at its segments' shape-determined lengths, one trailing unknown-length segment takes the remainder), with a definite length overrun a hard clash and an undetermined split abstaining to the parking path. Solving-side only — the shared reduction and peel laws are untouched, and committed solutions make the folded spellings agree by plain reduction, so the kernel needs no matching rule | Nothing |
...
**Resolved without implementation, 2026-08-08.** The section's own re-measurement clause decided it: the measured chains were the defect ledger's unmaterialized-candidate bug, not missing pruning; with that fixed, the acceptance probe (`Future/new` with an implicit parameter, `Async.crs`'s two call sites dropping the argument, `Future.crs:13`'s justifying comment deleted) landed against the unmodified guard, and no corpus site remains whose equation needs a restriction-commit rather than a wake. Implementing restriction in the file with the most perimeter weight, for zero consumers, fails this document's own demand bar — real pruning is dropped with the same standing as η-metavariable heads: reinstate when a real program blocks on an equation *solvable only by restriction* — one whose embedded metavariable never receives a solution by any other route yet has a legitimate narrowed solution. The section below is the original design, kept until retirement as the reinstatement's starting point.
...
**Landed 2026-08-08** (`packed_literal_view`, beside the shared peel it complements), with all sixteen `succ.crs` arguments dropped. The implementation sits entirely in `curios-elab`'s intrinsic congruence — the shared `spine.rs` peel and the reduction laws are untouched, which is narrower than this section's rigid-rigid framing anticipated. The section below is the original scope, kept until retirement.
