...
**One thing found and not fixed here.** `suggest.rs`'s pools are bounded by what a program already mentions, which is a suggestion-quality question rather than a scope-correctness one, and which the unit boundary sharpens rather than creates. It is recorded in full under [Findings that belong to no phase](#findings-that-belong-to-no-phase), with the cheap correction and the real question stated separately.
...
**Goal suggestions are bounded by what a program already mentions, and a unit boundary is where that starts costing.** Found by [B5](#b5--every-all-the-names-in-the-program-site-becomes-a-scope-question)'s audit, recorded here because it is a suggestion-quality question rather than a scope-correctness one — every site that pass examined was correct.

`suggest.rs` ranks five pools: scope values, constructors, scope functions, the module's own definitions, then *the globals the module already references*. "Scope" in the first three is the goal's own binder telescope, not the compilation scope, so **the only route by which a predecessor unit's definition can ever be suggested is that the program mentions it somewhere else already**. Nothing in `/std` you have not already used can be offered for a hole, and after [A5](#a5--more-than-one-unit) the same is true of every mounted unit. That is a deliberate relevance bound rather than an oversight — the alternative pool is every definition in scope, which is thousands, against an `ATTEMPTS` cap of 128 — but it is the bound that decides how useful a hole is in a program with dependencies, and it was chosen when the only predecessor was one nobody expected to be searched.

Two things to weigh if it is taken up. The cheap correction: the reference walk collects each item's type and body and the entrypoint's *body*, but not the entrypoint's *type* — so a program with a written entrypoint annotation loses one source of candidates for no reason anyone chose. `reach.rs` walks both, which is what makes the omission visible as an omission. The real question underneath: whether ranking by type shape — indexing the scope's definitions by their instantiated result head, so a goal of type `Foo` reaches the functions that produce a `Foo` without needing to have mentioned them — is worth building, which is the standard answer and is what makes the pool bounded by relevance rather than by reachability.
