...
**The substrate.** A `Lattice` trait — bottom, join, and the partial order the join induces — plus a worklist solver parameterized by propagation direction, with facts keyed by `CpsValueId`, function parameter, and continuation parameter. It absorbs `invariant_fixpoint`'s loop and `scc_invariant_knowns`'s recursion handling, and it runs `analyze_calls` **once per round**, handing the same `CallAnalysis` to every client instead of each pass rebuilding it.

Facts are keyed per value rather than per program point. In CPS this is nearly free and it is what makes the substrate small: a continuation's parameters *are* the join points and the term nesting *is* the dominator tree, so the structure a dataflow framework normally computes is already syntactic. Path-sensitive refinement — knowing a scrutinee's tag inside a `Switch` arm — is deliberately out of scope; it is a per-program-point extension the keying can grow into, and no client below needs it.
...
- **M1 — the substrate, with constant propagation re-hosted.** The `Lattice` trait, the solver, the once-per-round `CallAnalysis`, and `known_values`/`scc_invariant_knowns` moved onto them. *Acceptance: the specializer's decisions and the emitted module are unchanged.* Lands as its own commit, with no user-visible payoff, deliberately.
