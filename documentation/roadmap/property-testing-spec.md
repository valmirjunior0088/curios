...
A test takes no parameters, and a claim about every instantiation is a `let` whose type states it and whose body proves it ([A test is a check that runs, and a proof is a `let`](../design/language/a-test-is-a-check-that-runs-and-a-proof-is-a-let.md)). That leaves one thing unsaid. A claim that is *true* but not a theorem the kernel settles — `List/sort` returns a sorted list, a decoder's remainder is a suffix of its input — can be checked only at instances the author writes out by hand, as a `let` scheduled through `Test/all` over a table.
...
- It was the sole consumer of witnesses keyed on a function type, a language feature retired with it — so a replacement wanting one would have to argue that key back in.
...
- **A conditional property was never expressible.** `test bounded(n: Nat, p: Nat/Lt(n, 100))` had no witness, so a claim over a constrained domain could not be stated; today it has no spelling at all. This is the gap where a dependently typed language should be ahead of QuickCheck, and the published route is compiling an inductive relation into a generator by narrowing.

## The seam it comes back through

Three places, and no more: the `test` declaration's surface in `curios-text/src/parse/top_level.rs` and the tail it schedules in `curios-elab/src/builders.rs`, the `TestSyntax` slots in `curios-utilities/src/syntax.rs`, and the shape of a generator concept under `/std/Test`.

## Open questions

- Is the replacement a second meaning for a `test` declaration — which this design deliberately has none of — or a combinator an ordinary test schedules, the way a table already is?
