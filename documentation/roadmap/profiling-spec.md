...
- **No span survives below Core.** None of `curios-ersd`, `curios-cont` and `curios-emit` mentions one, which is why the unit of attribution is the declaration and not a position. Identity survives instead, as [one naming scheme for compiler identities](../design/toolchain/one-naming-scheme-for-compiler-identities.md) states.
