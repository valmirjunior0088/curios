...
**A one-sided check reads exactly like a passing one.** The bit-grain arms were not unchecked. `tests::laws` stated the free monoid's rows and every row it stated passed — but it stated most of them at the byte grain alone, and `eql` at neither, so the grade stayed **argued** while a green suite said nothing was wrong. It then happened twice more, one carrier over: `ListGet` had no seam rule where `ListSlice` had one, and `list_concatenated` was shallow where its `Bin` twin flattened, so `List` located a window at a symbolic seam and not an index at the same seam — a live incompleteness rather than a tidiness point. Neither was visible from reading the twins side by side; the grid found both, and only once the rows were stated at `List`. State a law at every carrier before unifying anything, and state it first: that is what turns each later step into a refactor with an oracle rather than a change with a hope.

**A reader folds into a carrier exactly when it needs nothing the carrier does not already hold.** Eight `bin_*`/`list_*` pairs became methods on `FreeMonoid` over two seams — `spine` for a *measurable* node, `joined` for a juxtaposition, which differ in what they admit rather than in how they recurse. Two pairs stayed split and the reasons are recorded beside them: `bin_shape` and `list_shape` answer different types and only one is fallible, and `bin_piece` and `list_piece` each need something from outside the value — a grain is a tag the carrier holds, a `List`'s element type is a term it does not. The criterion is worth keeping because it is decidable in advance: it says which of the remaining copies are worth attacking without re-arguing each one.
