...
**`seam_window` compares whole terms.** The one locator that accepts a symbolic position walks a concatenation accumulating a prefix sum, rebuilding it as a `Term` and re-reducing it at every operand, and matches it against the target by structural equality — so two sums definitionally but not syntactically equal decline. The re-reduction is a charge against the step budget; it is not where a deep window's time goes, which is the next paragraph's subject and was first read the other way round.
...
**The superquadratic growth belongs to the harness, not to the locator.** A window on the last seam of a symbolic concatenation, at 16, 32, 64 and 128 operands, grows by between three and five and a half times per doubling against a baseline without the window — but the generator scales the declaration's binder count and the size of its dependent function type along with the operand count, so that is what the growth measures. A control at the same binder counts carrying the same sum, with no concatenation and no window at all, grows the same way and costs *more*. Profiled at 128 operands: building and sorting the function type and checking a body against it is about 1.8 of the 2.9 seconds over baseline, `Nat`'s sum normal form is under a tenth of it, and the locator's own accumulation and comparison do not appear. Rewriting the accumulation moved no figure, which is how this was found. The remaining cost is elaboration's, is reached by any declaration with enough binders over a large enough type, and is [its own roadmap row](../roadmap.md) rather than this specification's.
...
**The prefix walk is one pass.** The distance still to cover is held in the sum normal form and each operand's measure is cancelled off it by `Nat::cancel_common`, which reads that operand's own summands rather than every summand before it — matching by *consumption*, an exhausted distance being the seam. What `cancel_common` leaves on the right is what the measure carried and the distance did not, so an overshoot arrives as a residual to read rather than a difference to guard, and no subtraction can underflow. Stopping at the first overshoot is exact rather than conservative, because a prefix only grows and a passed seam cannot come back. Measuring an operand still reaches the reducer; the accumulation and the comparison no longer do, and neither does the window's end, which is never built. **The reason to take it is the admission surface, not the cost** — that was the other way round when this was written, and the paragraph above records the measurement that turned it: the comparison it replaces is whole-term structural equality, while a linear combination keys its summands up to erased universes, so two spellings that decline today match. That is the admitting direction, on the same licence the refinement key records for `Nat` summands, and the perimeter entry stating the structural criterion as its evidence is rewritten with it.
...
1. The prefix walk and `seam_window` over it, with the perimeter entry rewritten. First because it is orthogonal to everything after it and the likeliest to surprise — which it was, though not where expected: the widening needed only the entry, and the cost it was thought to carry turned out to belong to elaboration.
