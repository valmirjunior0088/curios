...
- **A type-level walk must be structural** (probed). `/std/List/find` in a `Prop`-valued function is refused as "not known to terminate", because it is an index loop under an invariant rather than a fold. The same `find` written as a case split with general recursion on the tail — `[h, ..t] => match pred(h) | true => some(h) | false => find(t, pred) end` — keeps the early exit and is accepted in a type position, under the rule that a call on a constructor payload bound by the pattern decreases.
...
- **A spec entry may carry its value type** (probed, run). `struct Value { A: Type, metavar: Str, read: (Str) -> Result(A, Str) }` is accepted and `Value { A = Nat, … }` constructs it, so a reader is data rather than a witness — optparse-applicative's `ReadM`, cmdliner's converters and cmd-ts's `Type` — and a program's own type joins a spec by supplying one.
...
- `Value` — `{A: Type, metavar: Str, read: (Str) -> Result(A, Str)}`, with `str`, `nat`, `int` and `flt` supplied and `reader(metavar, f)` for a program's own type.
...
`/std/List/find` is rewritten as the case split above: the same signature, the same early exit, no index loop and no invariant, accepted in a type position. `http.crs` is its one consumer and is untouched. The indispensable-tier specification's account of `find` as an index loop is updated in the same change. Pinned by a test that demands a `Prop` computed through `List/find` at a literal.
...
- **The tokenizer carries private prefix helpers over `Str/try_get` and `Str/slice`.** `starts_with`, `split_once` and the rest are the indispensable tier's item three. When it lands, the helpers are deleted in favour of it.
