...
`/std/List/find` is rewritten as the case split above: the same signature, the same early exit, no index loop and no invariant, accepted in a type position. `http.crs` is its one consumer and is untouched. The indispensable-tier specification states this shape as the one every predicate function on `List` takes. Pinned by a test that demands a `Prop` computed through `List/find` at a literal.
...
- **The tokenizer carries private prefix helpers over `Str/try_get` and `Str/slice`.** `starts_with`, `split_once` and the rest are the indispensable tier's item one. When it lands, the helpers are deleted in favour of it.
