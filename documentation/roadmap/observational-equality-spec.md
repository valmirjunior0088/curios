...
**7. What is `~` at `Flt`?** [`Flt` is opaque at the type level](../design/language/flt-is-opaque-at-the-type-level.md) and its documented semantics is bitwise identity. Whether that *is* the observational equality, or whether an opaque carrier should have a primitive `~` with no structural clauses, is open.
