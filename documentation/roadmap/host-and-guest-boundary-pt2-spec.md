...
`for_each_host_op!` is a table of typed rows — `HandleRead: fn handle_read(h: Handle, n: u64) -> Result<Option<Vec<u8>>, Failure> as Handle/read { yields: bytes }` — and generates `HostOp`, an archived `Copy` enum with one variant per row, the wire name in CamelCase, with `ALL`, `named`, `name`, `subject`, `label`, `signature`, `outcome`, `diverges`, `description` and — once checkpoint 6 completes — the contract's failures and checks. A variant is nothing more than the row it names; consumers read an operation's contract rather than matching on operations. `ForeignFunction` becomes `Builtin(HostOp)` or `Declared(DeclaredForeign)`, read through accessors; only a declared `ffi` row carries its own signature. A builtin's signature and contract are read from the roster wherever they are needed, so no copy exists to disagree with it: equality, interning, linking and cache admission need no agreement check, because a conflicting description cannot be written. `host_ops()` remains the store of every builtin, and the runtime links by the same identity.
...
The checked host interface presents meaningful arguments and outcomes. Each row is written in the types its `HostOps` method takes and returns, and the wire data is read off those types by `curios-abi`'s `WireOperand`, `WirePayload` and `WireReply`: `Result<Handle, Failure>` for opening, `Result<Option<Vec<u8>>, Failure>` for reading, `Option<Vec<u8>>` for environment lookup, `Result<(), Failure>` for a status-only row, the named structs `Timestamp`, `TtySize` and `FileStat` for several success fields, `ChildExit` for a child's end and `Termination` for exit. Operands arrive owned, so a host may keep a buffer without copying it. A lone payload crosses under the label the row's `yields` names. These are shapes around ordinary buffers, not new wrapper types for them. Use existing domain types where they establish a useful invariant; do not generate guest declarations from the Rust host vocabulary. The same table generates the runtime's bindings, so a binding's types are its row's.
...
`Lift` and `Lower` each state the wire shape they decode or encode, and `ForeignBindings::define` asserts that shape against the row it binds, which is what holds an embedder's `ffi` bindings to their rows; a builtin's binding is generated from its row, so its shape is the row's by construction. `bool` replaces `u32` for a `Bool`. Closed argument codes decode into their own types — `Mode`, `StdioMode`, `SerialParity`, `SerialFlow`, `SerialOp` — and a poll's interest bytes decode as masks within `read | write`. Decode and check raw arguments before invoking the host method. Keep raw binding access internal to the runtime.
