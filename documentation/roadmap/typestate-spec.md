...
**Indexed monads.** Atkey's parameterised monads, generalised dependently by Brady's `Control.ST` — everything that would need linearity becomes part of the monad's state, so the resource is never a value and there is nothing stale to hold. This is the complete answer and the only one that recovers incremental control. It is blocked on the language, not on the library: `/std/Monad` is `Monad(M: (Type) -> Type)` and types `bind(m: M(A), f: (A) -> M(B)) -> M(B)`, while an indexed bind is `M(i, j, A) -> ((A) -> M(j, k, B)) -> M(i, k, B)`, which is not an instance of that shape — and `!` resolves sequencing through `Monad`, so an indexed monad would get no sugar. A second concept and `!` support for it are what this costs.
