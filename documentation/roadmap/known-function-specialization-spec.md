...
- **Dispatch is a table index, not a reference.** [A closure carries its code as a table index](../design/toolchain/a-closure-carries-its-code-as-a-table-index.md) is landed: construction writes an `i32` code field, dispatch reads it into `call_indirect`, and `curios`' `tests::codegen::structural::closure_tests` asserts `call_ref` is absent from a module entirely. **The ladder's per-character table names `call_ref`, which the emitted module no longer contains** — read the emitted body rather than that row when retaking the measurement.
