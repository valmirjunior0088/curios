...
Researched and prototyped, not designed. This specification records what a survey of the field settled, what three prototypes measured against the compiler this checkout builds, and the decisions that have to be answered before a `/std` module is written. The toolchain defects met while prototyping are fixed, and `/std/Html` exists as the tree a template of that kind renders into; the module itself is not started.
...
- **Escaping selects by kind through a concept keyed on the value's type.** Prototyped as a two-parameter `Into(A, K)` with `K` an output kind, keying on both heads, so a `Str` is escaped into `Html`, an `Html` passes, and a `Nat` shows; the same `Str` into `Text` passes. The method's result must be the kind: a concept parameter the method type does not mention is never pinned, and resolution waits forever on it. With `/std/Html` now a tree rather than a string, the Html kind's concept is `Html/Embed(A)` with `embed(A) -> Html`, and the escaping is `Html/render`'s rather than the template's.
...
**Kinds.** A template declares its kind by the function that makes it — `Html/template`, `Text/template`. A value enters an `Html` hole through `Html/Embed(A)`, whose method yields a tree: a `Str` becomes `text`, an `Html` passes through, a `Nat` shows into text, and a filled template is a `fragment`. The rendering is `Html/render`'s, which escapes every text node and attribute value, so there is no `raw` and no trust to state: a value that must land unescaped has nowhere to land, which is the decision `/std/Html` took. A program writes `Embed(Its)` for its own types under the orphan rule as it stands; a `Text` hole takes a value through `Show`. No contextual inference across script, style and unquoted-attribute positions in the first cut; whether the parser refuses a hole in those positions is decided below.
...
- **Naming.** `/std/Template` beside `/std/Html`, or `Html/template` as an entry of the tree's own module with `Text` elsewhere. Whether `Text` is a kind or the absence of one.
...
Toolchain defects met while prototyping, each fixed since, and what stands in their place.

- **The block string literal.** `"""` and a newline open one, a newline and `"""` close it, the whitespace shared by its lines and the closer is stripped, and `curios format` prints it back as a block ([syntax.md](../syntax.md)). A one-line `"…"` refuses a raw newline. This is what a template is written in.
- **A stuck computed type is reported by its written type.** `Fmt/render(s)` with a runtime `s` names the type as spelled and that `s` is not known, rather than the unfolded body of `format_type_with`.
- **A diagnostic elides a long literal.** A literal past 64 characters is quoted by its head and its length.
- **An undischarged decided proposition is reported by its reduct.** `Has(remove(remove(holes("…"), "count"), "name"), "name")` reports as the `False` it reduced to, at the name that is missing.
- **A closed computation over a literal is evaluated once per phase, not once per mention.** A literal's certificate is built once, the reduction cache answers modulo universe levels, and the closed machine remembers a forced induction hypothesis, so a parse mentioned four times costs one per phase and a parser fold over `Nat` is linear where it was exponential. The figures under *What is certain* predate all three and are to be re-taken before the fill shape is chosen; the bind-by-name shape may now be affordable.
