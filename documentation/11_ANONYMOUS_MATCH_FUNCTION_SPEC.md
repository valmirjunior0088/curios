...
Because `curios-text` is in the browser compiler's dependency graph, also run the `curios-js` wasm32 build and matching `wasm-bindgen --target web` step described in `AGENTS.md`.
