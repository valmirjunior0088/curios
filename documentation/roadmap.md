...
- [x] Command-line interfaces (`/std/Cli`) — a specification is a list of `Arg` values, the record a line parses into is `Values(spec)` computed from it, `get(v, name)` is typed by a type-level lookup and refuses a name the specification does not contain, `WellFormed` is decided by reduction, and `parse`, `select`, `help`, `usage` and `report` read a line and render the screens
  - [ ] `Cli/main`, which the universe item below blocks: a specification-indexed function applied to a `Type`-carrying struct field has its instance levels read as constants, so `parse(cli.args, …)` handed to `cli.run` is accepted by the elaborator and refused by the kernel. A program writes the entry itself meanwhile, in the shape `/std/Cli`'s header fixes
  - [ ] A declaration's interface universe levels are minimized rather than generalized when conversion has aliased them onto an occurrence's: `finalize` protects the metas named in the type, but `minimize`'s `solve_flexible_in` reaches the representatives those resolve to, so a function mentioning one level in both its domain and its result lands at a ground level while every type around it generalizes (`curios-elab/src/universe_solver.rs`)
  - [ ] A projection is not graded below the struct it came from, so a well-formedness check cannot recurse through a struct's list field and subcommands stay one level deep (`curios-analysis/src/totality.rs`)
  - [ ] A numeral does not park against an expectation stuck on a metavariable, so an indexed constructor cannot take a literal and every `Values` is built by the parser rather than written (`curios-elab/src/typing.rs`)
  - [ ] An `induct` lowers its type formers into a `rec` group unconditionally, where `struct`, `concept` and `witness` test their member count — so every non-recursive inductive carries a group of one that means nothing. Removing it needs `elaborate_module_let` to generalize universes and to assume the name while the registry telescopes rebuild (`curios-text/src/into_core.rs`)
