...
- [x] The standard library's indispensable tier — what every one of nine surveyed peers ships
  ...
  - [x] `Key(Nat)`, over the minimal big-endian encoding `/std/Hash` already gave it
...
- [ ] Developer tooling
  ...
  - [ ] [A question costs what its edit reaches](roadmap/question-latency-spec.md) (an edit that reaches nothing still re-lowers, re-classifies and re-erases all of `/std`, 6.0 s in release, and a save re-checks text already checked)
    - [ ] A save of unchanged text answers without compiling, and diagnostics publish before the kernel and erasure finish
    - [ ] A question skips what only a build reads: the documentation record and the store's serialize-and-digest
    - [ ] Totality classification runs over the closure (not refined yet)
    - [ ] Erasure reuses the baseline's erased items (not refined yet)
    - [ ] Lowering and item ordering reuse unchanged modules (not refined yet)
