...
- [ ] [A reduction step costs what it builds](roadmap/technical_debt/01_priced_reduction_spec.md) — the step counter prices a transition the same whatever it constructs, so nothing bounds the memory a reduction allocates or retains; and it charges a memo hit the whole cost of the computation it replaces, so a budget is spent on work the kernel did not do
...
- [ ] [A string literal is checked once per use](roadmap/technical_debt/04_string_literal_cost_spec.md) — a `Str` literal's UTF-8 validity is discharged by running a fold over its bytes, and the kernel re-runs it at every use site, so a six-kilobyte string that a program actually uses does not compile
