...
- [ ] Three measured cliffs where an ordinary spelling costs superlinearly, or refuses, each with the probe that reproduces it beside it in `curios`' `tests::unfolding`, and each independent of the others. Two are closed. The second was that a combinator application written inside a closure body was reified without sharing, so a grammar's copies grew as `n² + 2` where the same applications written at item level cost `n + 2` — sixteen rules cost 258 copies and 24.23 s against 18 and 1.29 s. A replacement's residual group is now bound ahead of the item enclosing its candidate rather than inside the candidate's own block, and the two spellings agree to the copy at every size. The first was the kernel's case-refinement key: the kernel reduced a match scrutinee to key its case refinement, once per arm, where the elaborator records the written spelling and escalates only on a miss — fourteen ordinary combinator definitions refused on the reduction budget, and `recheck` was 94.1% of that compile against elaboration's 0.4%. It now keys the same two ways, and the same programs are flat at forty definitions. Read in stage order:
  ...
  - [ ] [The fixpoint costs more than everything it optimizes](roadmap/technical_debts/03-cont-fixpoint-cost-spec.md) — `curios_cont::optimize` is about nine tenths of an ordinary `Toml/decode` compile and super-quadratic in module size, on code that reaches for none of the above. Instrumented and located: the round count is the parameter-split count, because nine passes admit one candidate per call, and 57% of the time goes to passes rebuilding their whole-module analysis on rounds where they rewrite nothing. The cure is draining each such pass per call; no cure is started
