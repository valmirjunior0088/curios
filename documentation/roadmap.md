...
- [ ] [A reduction step costs what it builds](roadmap/technical_debt/01_priced_reduction_spec.md) — the step counter prices a transition the same whatever it constructs, so nothing bounds the memory a reduction allocates or retains. **The memo half has landed**: a `whnf`/`forced` hit spends no steps and those two tables are cleared with the budget, so a budget is no longer spent on work the kernel did not do, and the kernel's 8–16× divergence from the elaborator on the same program closed to 1–2×. Pricing construction, the allocation audit, the retention quota and the recalibrated default remain
...
- [ ] [A string literal is checked once per use](roadmap/technical_debt/04_string_literal_cost_spec.md) — a `Str` literal's UTF-8 validity is discharged by running a fold over its bytes at a cost linear in its length, so a large enough literal does not compile. The per-use multiplier the title names is gone with the item above's memo half; what remains is the per-character constant, and re-measuring the ceiling it now sets is that specification's own first step
