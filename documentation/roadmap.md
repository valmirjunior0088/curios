...
- [ ] [A monomorphic field carries its own type](roadmap/typed-heap-fields-spec.md) (the typed closure tables landed — monad_io −25%, parse_digits −15%, both per-dispatch libcalls gone — and the shape recorder with them; the census is taken, 78% of recorded fields monomorphic and the box/unbox class the largest static population everywhere, so what remains is the campaign it gates: family-keyed final structs, an `i8` tag, disjoint typed slots)
- [x] The map's remaining distance is decomposed before it is spent — the campaign that took the insert from 1309 to 744 ns (the single walk −27%, the two-way branch, exact tuple reads at −61% on `chain`) closed 2026-08-20 by decomposing what it left: the collector's share measured **nil** (`spines_collection_decomposition`), key construction **14 ns — under 2%** (`map_wall_key_share`), and the compare-chain residue closed by inspection, the optimized module's nine surviving `br_table`s all in string decoding and none in the descent. What remains of the insert is the uniform-representation tax, whole, owned by [A monomorphic field carries its own type](roadmap/typed-heap-fields-spec.md); the retired specification lives in git history, its figures beside the probes that retake them, and the cross-language run 09 capture is deferred to a later sitting
