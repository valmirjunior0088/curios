...
- [ ] [The map's remaining distance falls by a walk, a cast and a branch](roadmap/map-distance-spec.md) (partly landed 2026-08-20 — the immediate-payload miscompile and the single walk, the latter at −27% on the insert slope; the two-way branch remains, and the exact-tuple-read step is under review and expected to be declined, since the width-keyed tuple tower is what stops Binaryen from refining any field type. Originally refined, not started: `/std/Map` still walks twice per insert and remove; a `match` on a two-heap-constructor variant reads its tag through a supertype cast that is a host call on the pinned engine, and a two-way `match` is a jump table; and the immediate-encoded match aliases its payload to the scrutinee, which miscompiles a loop that does arithmetic on it — the soundness fix lands first and alone)
