...
- [ ] [A string literal is checked once per use](roadmap/technical_debt/04_string_literal_cost_spec.md) — a `Str` literal's UTF-8 validity is discharged by running a fold over its bytes at a cost linear in its length, so a large enough literal does not compile. The per-use multiplier the title names is gone with the item above; what remains is the per-character constant, which that item's frame row made the dominant term — a literal nests one reduction level per byte, and the ceiling is now under 6 000 characters where roughly 12 000 compiled before. Its *What spec 01 measured* section carries the figures and what they change about its own scoping
