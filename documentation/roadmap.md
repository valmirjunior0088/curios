...
- [ ] [The map's remaining distance is decomposed before it is spent](roadmap/map-distance-spec.md) (`spines` reads 744 ns/insert and 2.91× off Lean at run 08; the decomposition's first cut measured the collector's share nil — the remainder is mutator work, the nursery is demoted — leaving the key-share and compare-chain probes, the representation class handed to typed-heap-fields, and a run 09 capture for the cross-language claim)
