...
- [ ] Developer tooling
  ...
  - [ ] [A claim over a domain nobody enumerated](roadmap/property-testing-spec.md) (not refined yet — shipped once as `test name(params)` probed over drawn arguments, removed because the declaration's quantifier and the report's disagreed; a test takes no parameters now, and a claim over every instantiation is a `let` whose type states it)
  ...
  - [ ] [A profile is a fact about the program, not about the machine](roadmap/profiling-spec.md) (nothing profiles a Curios program; `curios profile` profiles the compiler and is removed)
