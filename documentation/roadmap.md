...
Specifications live under [roadmap/](roadmap). A campaign large enough to sequence gets its own directory; everything else is filed by kind. [`technical_debt/`](roadmap/technical_debt) holds a shortcut that costs something today and was recorded when it was taken, [`capabilities/`](roadmap/capabilities) holds something that does not exist yet and costs nothing until a consumer asks for it, and [`optimizations/`](roadmap/optimizations) holds a cost the compiler could remove from code that is already correct. Within a directory the numeric prefix is a reading order, and it is append-only: a landed specification leaves its number behind rather than renumbering its siblings.
...
- [ ] [A variant travels as its fields behind a constant discriminant](roadmap/optimizations/01_variant_width_spec.md) (a variant in compiler-coordinated flow still pays a heap object and a dispatch re-read: continuation scalar replacement declines the variant-width shape — the UTF-8 loop's scan rebuild is the retained cost — and the split-return protocol declines variant-width components, a decline the immediate encoding widened to every such family)
- [ ] [A pure program rebuilds what an impure one would mutate](roadmap/optimizations/02_death_birth_churn_spec.md) (death-birth overlap is the sanctioned idiom — spread updates, `/std/Map` inserts — and its cost is unmeasured: no harness workload exercises it against the Perceus-compiled Lean column, and no census locates it in real code; the specification is deliberately mechanism-blocking until both numbers exist)
- [ ] [A closure carries its code as a table index rather than a funcref](roadmap/optimizations/03_closure_index_dispatch_spec.md) (every closure construction pays wasmtime's per-store funcref-to-GC-heap intern — ~75% of a profiled monadic loop — and every unknown call reads the funcref back: an `i32` index dispatched with `call_indirect` measured 25.6× on the mechanism, and hoisting the `ref.func` recovers only 2× of it because the intern is per store; waits on `curios-wasm` modeling tables, active element segments, and `call_indirect`)
...
- [ ] Developer tooling
  ...
  - [ ] [Payload reuse](roadmap/capabilities/05_payload_reuse_spec.md) (an unchanged manifest target re-executes without recompiling: the precompiled `.cwasm` filed beside the units under the same address/record split, shared by `run` and `compile`)
