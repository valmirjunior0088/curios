...
- [ ] [A monomorphic field carries its own type](roadmap/typed-heap-fields-spec.md) (the representation campaign, three mechanisms cheapest first: type the closure tables so both per-dispatch libcalls die, generalize `FieldShape` into the full erased-shape recorder whose run over the corpus is the census, then key heap types by family — final structs, an `i8` tag, disjoint typed slots — with the census gating that third step)
- [ ] [The map's remaining distance is decomposed before it is spent](roadmap/map-distance-spec.md) (`spines` still reads 744 ns/insert and 2.91× off Lean at run 08, known only as a total: a collector-separating decomposition ranks four named classes before anything is spent, each class handed to the spec that owns it, and the cross-language claim waits on a run 09 capture)
