...
- [x] Recursion restored to the defunctionalized walks — twelve walks recurse inside `curios-utilities`'s `recurse` instead of driving explicit frame machines (see [Depth is bought with stack, not with hand-rolled frames](design/toolchain/depth-is-bought-with-stack-not-with-hand-rolled-frames.md))
