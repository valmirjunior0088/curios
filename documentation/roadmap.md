...
- [x] A monomorphic field carries its own type — every recorded field shape that can name a heap type now declares one: a family's tag packs into a byte, scalar payloads are raw, and lists, closures and nominal rows are declared at their own types, with heap types keyed by family or product schema rather than by arity. Three populations stay uniform for stated reasons rather than for want of work — a packed carrier and an immediate-encoded family are *sometimes* immediates, so no type names them, and a family-typed slot is admitted only where its row does not widen. The rationale, the rejected alternatives and the boundaries are [a field is declared at the carrier its shape names](design/toolchain/a-field-is-declared-at-the-carrier-its-shape-names.md); the figures live beside the probes that retake them in `curios/src/tests/codegen/`
- [x] The map's remaining distance is decomposed before it is spent — the campaign that took the insert from 1309 to 744 ns (the single walk −27%, the two-way branch, exact tuple reads at −61% on `chain`) closed 2026-08-20 by decomposing what it left: the collector's share measured **nil** (`spines_collection_decomposition`), key construction **14 ns — under 2%** (`map_wall_key_share`), and the compare-chain residue closed by inspection, the optimized module's nine surviving `br_table`s all in string decoding and none in the descent. What remains of the insert is the uniform-representation tax, whole, taken up and closed by the typed-fields campaign above; both retired specifications live in git history, its figures beside the probes that retake them, and the cross-language run 09 capture is deferred to a later sitting
...
- [ ] One measured cliff where an ordinary spelling costs superlinearly, or refuses, with the probe that reproduces it beside it in `curios`' `tests::unfolding` — and four closed, each independent of the others. The fourth was the index inversion: an ordinary `Eq(top(n), true)` proof over a web of boolean combinator definitions cost ×2.4 per definition in *both* checkers, 5.5 s at thirteen on one host, and the specification that recorded it put the cost to conversion. Conversion was innocent — weak-head-and-compare stops `true` against a stuck term at the heads. The inverter's `force` paid for two things under it, both in `curios-core` where the checkers share: `reduce_bool_binary` reduced both operands of a `&&`/`||` before it could know a stuck left had settled the fold, so the web's weak-head form was its full normalization with nothing remembering a local-bearing term; and the closed machine substituted a global's *value* for its name at every beta, so each definition's value held the previous one's twice, a graph whose tree was `2^n` that the unfold memo retained at the tree's footprint and the strategy's beta opened as a tree. A `&&` now reads its right operand only behind a literal left, and a bare name stays a name: the proof is 0.16 s at every size, `recheck` 13.6 ms where it was 5 842, and the retention ladder the fifth cliff stands on fell a thousandfold with it. What that left was the same web over `Nat`, ×2.2 per definition still, because a `Nat` fold's identity laws must read the right operand, so its reduct is a graph whose tree is `2^n` — and four walks saw the tree: the kernel re-deriving what it remembered nothing local-bearing of, the elaborator's cache refusing every term naming a universe metavariable, the conversion history's `capture`, and `Term::eq` against an equal distinct key. Each is closed where it stood — a memo for local-bearing reducts scoped to the equations in force, a cache that admits level metavariables since reduction is parametric in them, and two walks that remember what they have entered — and the `numeric, proved` rows of `scrutinee_refinement_measurements` read 0.15 s at every size, a 106 ms compile at thirteen where it was 4.5 s. The third was the Cont fixpoint, which cost more than everything it optimized — 97.6% of an ordinary `Toml/decode` compile when first measured, super-quadratic in module size, on code that reached for nothing unusual. Two stage-level spans could not say why; a span and a fired-sample per pass could: the round count was the count of one-candidate-per-call rewrites, three passes deep, and most of the time went to passes rebuilding their whole-module analysis on rounds where they rewrote nothing. `split_parameters`, `eliminate_dead_parameters` and `contify_calls` now drain every candidate one snapshot admits, and `forward_aggregate_projections` forwards every projection in one sweep — 57 rounds and 2 858 ms to 11 rounds and 634 ms on that program, its share of the compile from 89% to 67%. `curios`' `tests::fixpoint` carries the before and the after, and what remains is inside passes rather than between them: the inliner's three body walks per candidate site, the splits' one module walk per split, and `into_wasm` at 12% of the compile, which the fixpoint used to hide. The second was that a combinator application written inside a closure body was reified without sharing, so a grammar's copies grew as `n² + 2` where the same applications written at item level cost `n + 2` — sixteen rules cost 258 copies and 24.23 s against 18 and 1.29 s. A replacement's residual group is now bound ahead of the item enclosing its candidate rather than inside the candidate's own block, and the two spellings agree to the copy at every size. The first was the kernel's case-refinement key: the kernel reduced a match scrutinee to key its case refinement, once per arm, where the elaborator records the written spelling and escalates only on a miss — fourteen ordinary combinator definitions refused on the reduction budget, and `recheck` was 94.1% of that compile against elaboration's 0.4%. It now keys the same two ways, and the same programs are flat at forty definitions. Read in stage order:
  - [ ] [The retention allowance is a compilation's, and one declaration can spend it](roadmap/technical_debts/05-kernel-retention-accounting-spec.md) — thirteen definitions of the above consumed 31% of the whole compilation's retention quota and three such proofs consumed it all, after which every memo insertion is refused and the compile stops being linear in what it spends. Where those units went is found — the closed machine's value-substituted reducts, priced by `footprint` as the tree of a graph — and closed with the fourth cliff, so the proof retains a thousandth of what it did. What stands is the allowance itself: compilation-scoped, never refunded, unobservable when crossed — the elaborator's `Str` share went with the conversion that stepped a folded scan one unfolding per round
