...
- [x] A string literal is checked once per use — a `Str` literal's UTF-8 validity is discharged by running a fold over its bytes, and the per-use multiplier the title names went with the item above. What decided the rest was not in `/syn/Str` at all: the kernel's evaluation memo was consulted only at its crate boundary while the elaborator's was consulted at every reduction level, so the kernel charged **5.3× the elaborator for the identical reduction at the identical depth**. `whnf_within` now probes and stores at every level, the two checkers agree to 1.0× at every size, and a character costs 1 088 units of which 1 024 are the frame row — so a literal is priced at one guarded reduction level each and the ceiling rose from about 5 200 characters to **about 16 700**. Slicing was never implicated: `Str/slice` supplies its bounds with `@drop_width_within` and costs 0.1% over the bare literal. `curios`' `str_literal_cost_measurements` carries the table, `a_str_literal_costs_about_one_frame_per_character` holds it, and what remains open is the item below
