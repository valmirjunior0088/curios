...
- [ ] [Restoring recursion to the defunctionalized walks](roadmap/compiler/11_STACKER_MIGRATION_SPEC.md) — explicit frame machines keep data-shaped depth off the native stack, which `curios-base`'s `recurse` now does without trading away the legibility two independently-written checkers depend on. Ten walks are restored — the kernel's three (`convert`, `infer`, `whnf`, −242/+131), then `curios-elab`'s reducer, both universe-solver walks, the printer's document construction, the kernel's satisfiability search, the erased-module verifier, and the size-change totality walk — leaving the lowerer's `let` sites and the elaboration fast path. `infer` established the stake: its worklist had silently dropped the three *checked* rules from every argument, payload and field position, and passed every gate for two weeks because the corpus never reached the shape. A workspace sweep closed the inventory: four machines lay outside it, three are restored, and only `curios-cont`'s CSE remains. `Term::walk` stays a shared driver, and its internals are deferred indefinitely: the stack was never what it is for — it exists for its child-enumeration fold and as the seam each analysis hangs its own memo on, and memoization is what the measurements behind it actually cured.
