...
- [ ] [Ersd v2](01_ERSD_V2_SPEC.md) (the semantic half of the backend, symmetric to Cont v2: a flat, verified, first-order erased representation that delays desugaring until semantic work is finished; erasure as pure transcription under the once-per-expression operand law, the three forced transformations — eager-item pruning, partial evaluation, monoid worker/wrapper — and a single normative lowering into the landed continuation interface, replacing the legacy erased-term representation outright)
