...
- [x] Intrinsics as orthogonal builtins _(uniform `/sys` builtin declarations, not parser-special-cased)_
  ...
  - [x] `List`
- [x] Total `/sys` primitives (every operation whose reduction could fail now carries its precondition in its type, in the decided style, so a literal argument discharges definitionally and only a variable one demands evidence: `get` and `slice` behind their index and window bounds, `div`/`rem` behind a `Prop`-valued `Ok` field on the `Div`/`Rem` concepts, `Int/to_nat` behind non-negativity; trivial inhabitation fills a goal already reduced to truth, without which a proof-carrying primitive is unusable at a literal argument; the `/sys` provenance gate re-scoped onto interface stability, correctness having moved into the types; and `/sys/Io`'s roster asserted to offer no eliminator)
...
- [x] A compilation is units folded over a dependency order (the compiler no longer structurally knows there is a prelude and a program: every stage takes a scope of N predecessors borrowed rather than one merged map; a root is the mount prefix that owns a name, found by longest match, with `RootId`/`of_segment` and all five archived `root` stamps deleted rather than extended; one lowering and one erasure where there were three and two; the compilation unit named in `curios-unit`, a crate the kernel does not reach; `curios-pipeline` carrying no standard-library edge outside its fixtures; and `--unit <DIR>` mounting a second unit — a package directory, whose own manifest declares the prefix it mounts at — against which the orphan rule fires and a prefix collision is refused naming both claimants. What a *project* is — manifests, pinned dependencies, and the unit cache — is specified separately)
...
- [ ] [Rebasing a fold out of the endomorphism monoid, so a string walk stops allocating per character](roadmap/compiler/10_ERSD_ENDOMORPHISM_FOLD_SPEC.md) — `/std/Str/fold`'s motive is a function, because the walk carries a `Utf8(sc, b)` proof indexed by the scan state and a right fold cannot thread an accumulator leftwards without one. So it builds `step₀ ∘ … ∘ base`, one closure per byte, and applies it once. Functions under composition are a monoid, and `curios-ersd` already reassociates monoid-deferred recursion — the endomorphism row and the `FoldSequence` target are what it lacks. **The survey is why this is not simply built:** two `fold-seq` nodes exist corpus-wide and no `fold-nat`, always the same two, and only one is rebasable — `/std/Str/utf8/check` applies its hypothesis and then matches the result, a deferred context rather than a composition. One site, in `/std`, from a shape the library chose; so the specification recommends reformulating that function before adding an arena phase to serve it.
...
- [ ] [The unfolding discard decides on shape where it means to decide on progress](roadmap/compiler/12_REC_UNFOLDING_DISCARD_SPEC.md) — `force_rec` grants a folded recursive call one unfolding and then discards it when the reduct's *head* is a `Var`/`Match`/`Metavar`/`Proj`, which conflates "made no progress" with "finished, and the answer is a variable". So `go(0, acc)` reduces to `acc` and the answer is thrown away, shutting the prove-it-by-induction hatch for any bound over a program-local `rec` whose base case returns a parameter. Conversion reaches the same unfolding through `unfold_rec_apply`/`unfold_spelling`, which do not discard, which is why `Eq` sees through what `Nat/Le` cannot. Refusing, never unsound; the clause is load-bearing against unfold-and-restuck and must not simply be deleted.
- [ ] [Restoring recursion to the defunctionalized walks](roadmap/compiler/11_STACKER_MIGRATION_SPEC.md) — explicit frame machines keep data-shaped depth off the native stack, which `curios-base`'s `recurse` now does without trading away the legibility two independently-written checkers depend on. Twelve walks are restored — the kernel's three (`convert`, `infer`, `whnf`, −242/+131), then `curios-elab`'s reducer, both universe-solver walks, the printer's document construction, the kernel's satisfiability search, the erased-module verifier, the size-change totality walk, the lowerer's `let` blocks, and the elaboration fast path (−257/+23, deleted on the evidence that disabling it leaves the whole suite green). `infer` established the stake: its worklist had silently dropped the three *checked* rules from every argument, payload and field position, and passed every gate for two weeks because the corpus never reached the shape. A workspace sweep closed the inventory: four machines lay outside it, three are restored, and only `curios-cont`'s CSE remains. `Term::walk` stays a shared driver, and its internals are deferred indefinitely: the stack was never what it is for — it exists for its child-enumeration fold and as the seam each analysis hangs its own memo on, and memoization is what the measurements behind it actually cured.
