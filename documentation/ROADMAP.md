...
- [ ] [A dataflow substrate for `curios-cont`, and unboxed scalar locals as its first payoff](roadmap/compiler/10_CONT_DATAFLOW_SPEC.md) — three of its four milestones landed, and what remains is M4, demand analysis: a backward walk of which parameters and results are consumed, subsuming `eliminate_dead_parameters`. Landed: the lattice and SCC fixpoint welded to the specializer extracted into a shared solver (`cps/dataflow.rs`), the per-operand representation demand lifted from 130 emitter sites onto the intrinsic roster as `Repr`, and scalars kept in registers across a function body (`cps/represent.rs`), which moved the integer benchmark for the first time in five runs and left the allocation one flat, with the emitted module validated before Binaryen sees it. Two claims this entry used to make were withdrawn by the work itself and are recorded there: hoisting the call-graph rebuild out of the per-round loop is unsound while every pass takes `&mut CpsModule`, and the 64-bit multiply belongs to the `i31` overflow check rather than to boxing, so no representation decision retires it.
