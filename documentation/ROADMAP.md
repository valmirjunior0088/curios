...
- [ ] [Returning through several continuations, so a tagged union becomes control flow](roadmap/compiler/10_CONT_RETURN_PROTOCOL_SPEC.md) — a variant lowers to a heap tuple `(tag, payload…)` that its caller projects and switches on, and four existing passes already erase construct-then-match whenever value identity survives. It does not survive a call: the join specializer inspects `ApplyCont` and `Switch` edges, a call is neither, and a returned constructor is therefore opaque at its caller. Contification restores identity only for a callee with one external call site, and inlining declines a recursive one, one holding a nested definition, and a multi-site one above eight nodes — so a recursive-descent parser returning `Option` is excluded on every count, which is where `/std`'s 88 such return sites concentrate. Carrying the caller's switch back to the callee's return edges removes the allocation and the tag test together. Demand analysis — which results a caller actually consumes — is the prerequisite and subsumes `eliminate_dead_parameters` on the way; the return protocol itself is deliberately left unscoped until a `Stage::Cont` survey reports which tier the surviving sites are in, and until raising the inline bound is ruled out as the whole answer. Deliberately **not** in scope: the closure indirection `!` generates, whose blocker is at the argument boundary rather than the return, and which the specification names in its successor.
