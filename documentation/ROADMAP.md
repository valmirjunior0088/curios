...
- [ ] [Restoring recursion to the defunctionalized walks](roadmap/compiler/11_STACKER_MIGRATION_SPEC.md) — six walks still drive explicit frame machines to keep data-shaped depth off the native stack, which `curios-base`'s `recurse` now does without trading away the legibility two independently-written checkers depend on. The kernel's three landed together (`convert`, `infer`, `whnf`, −242/+131), and one of them established the stake: `infer`'s worklist had silently dropped the three *checked* rules from every argument, payload and field position, and passed every gate for two weeks because the corpus never reached the shape. `Term::walk` is explicitly out of scope and stays a shared driver.
