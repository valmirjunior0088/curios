...
- [ ] [Certified archives: separating the image from the verdict](roadmap/compiler/10_CERTIFIED_ARCHIVE_SPEC.md) (`curios-archive` owns elaboration and the image while `curios-prelude` keeps its name and certifies the restored bytes, so each half is invalidated by what it depends on; then parallel per-item certification, and cached user modules once binder identity and prefix identification are defended well enough to carry it)
