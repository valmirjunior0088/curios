...
- [x] Intrinsics as orthogonal builtins _(uniform `/sys` builtin declarations, not parser-special-cased)_
  ...
  - [x] `List`
- [ ] [Retiring `/sys`'s partial primitives, and the obligations that replace them](roadmap/compiler/11_TOTAL_PRIMITIVES_SPEC.md) — ten operations whose reduction can *fail*, each taking its precondition into its type in the decided style, so that a literal argument discharges definitionally and only a variable one demands evidence. `get` keeps its `Nat/Lt` obligation and stops leaking the raw intrinsic past it, which is the gap `/std/Map` and `/std/List/find` both walked through. **One claim this entry made has already been withdrawn by the work and is recorded in the spec:** `slice` was to clamp, as the canonical extension `Nat/sub` already takes, making window fusion unconditionally true — but the spine fuses on a matching seam with no ordering check, so clamping turns `aggregates.rs`'s universally-quantified `split` into a proof of a false equation. `slice` takes a precondition too, and the criterion that produced the wrong answer is what caught it. `div`/`rem` take their domain from a `Prop`-valued field on the `Div`/`Rem` concepts rather than the `x/0 = 0` junk value Lean and Isabelle adopt, whose reasoning inverts here: a proof erases, while a junk value costs a guard branch on every division since `i32.div_u` traps. Enabled by trivial inhabitation — an implicit whose type reduces to a nullary single-constructor proposition solved with that constructor, verified absent today and the one thing without which a proof-carrying primitive is unusable at a literal argument. Also re-scopes the `/sys` provenance gate onto ABI stability, correctness having moved into the types, and asserts the two Rust tables *A type is a pure term* rests on.
