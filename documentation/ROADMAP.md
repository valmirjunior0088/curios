...
- [ ] A copied body reproduces the definitions nested inside it — `clone_scc`, `clone_continuation`, and `inline_known_calls` are three near-verbatim subtree copiers that each decline outright when the body nests a `LetFun` or `RecInit`, because a verbatim clone does not reproduce a nested definition. A nested lambda is what higher-order code is made of: `/std/Str/fold`'s body *is* one, and `programs/parse_digits.crs` carried 34 such nodes when surveyed at `d771eaac`. Collapsing the three into one copier and closing its member set under lexical nesting lifts the refusal once rather than three times, after which the inliner's size test has to count the members it would duplicate — `function_nodes` walks a `LetFun`'s continuation and not its members, which cost nothing while such a body was refused outright and understates the duplication the moment one is admitted. Changes what the existing inliner and both specializers reach across the whole corpus, most of which is not higher-order code, so it is measured rather than verified. Blocks the specialization below, and was invisible until that work went looking for a floor.
- [ ] [Specializing on a known function, so an abstraction stops costing an indirect call](roadmap/compiler/10_CONT_HIGHER_ORDER_SPEC.md) — there is no dictionary passing to remove: a `satisfy` witness is resolved during elaboration and inlined, and no `Monad` survives into the optimized CPS of either monadic program. What survives is a function reaching its call site as a value, at two boundaries that are not one problem. As an *argument*: `/std/Str/fold` is called from two places with two different step functions, and every pass declines structurally — the SCC specializer needs a self-edge `fold` does not have, since its recursion runs through closures; both call-pattern specializers key on a tagged tuple and a function reference is not one; contification needs a single call site; inlining refuses a multi-site callee above eight nodes. As a *return*: a `State` action is a function, so the loop returns one of two function references and applies the result of a call, which no pass reaches for the same reason the return protocol was written. Nothing downstream is missing — `rewrite_atoms` already devirtualizes the moment a callee value is known — so the argument boundary is two existing passes recognizing one more atom, once the copier above admits the shape. The return boundary is not scoped, and the specification carries a stopping rule that lets its survey retire it unbuilt: `state_monad` and `state_manual` differ only in abstraction, and if the argument boundary closes that gap there is nothing left to build.
