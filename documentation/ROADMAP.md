...
- [ ] [Moving an application into the function that returns it, so a monadic step stops allocating](roadmap/compiler/10_CONT_UNCURRY_SPEC.md) — a `State(S, A)` is a `(S) -> {A, S}`, so an action *is* a closure and every `!` allocates one and calls it indirectly: twenty million steps cost 1.08s against a hand-threaded 0.01s, about a hundredfold and the largest cost measured here. Neither the witness nor the desugaring is to blame — no `Monad` survives elaboration, and a fold with `!` written out as a match times identically to one without — so what costs is the carrier's closure. It is never needed: across four programs, every function returning a bare function reference has every non-tail use immediately applied, zero exceptions, and the four that appear in *all* of them are `Io`'s description machinery rather than anything monad-specific. So the application can move into the callee, which needs no knowledge of *which* closure comes back and is therefore strictly smaller than the defunctionalization the previous specification planned. One more point on the return protocol's demand lattice states it, and the same tail-call components decide it.
- [ ] Specializing on a known function argument, so a shared combinator stops calling through a parameter — `/std/Str/fold` is called from two places with two different step functions, so the parameter joins to a conflict and every call through it stays indirect, once per character. Every pass declines structurally: the SCC specializer needs a self-edge `fold` does not have, since its recursion runs through closures; both call-pattern specializers key on a tagged tuple and a function reference is not one; contification needs a single call site; inlining refuses a multi-site callee above eight nodes. Nothing downstream is missing, since `rewrite_atoms` devirtualizes the moment a callee value is known — so this is two existing passes recognizing one more atom. Bounded above by a hand-monomorphized control at roughly eightfold, though that control also skips the UTF-8 scan `Str/fold` performs, so the recoverable share is smaller and unmeasured. Ordered after the uncurrying above, which targets the larger gap and is the cheaper build.
