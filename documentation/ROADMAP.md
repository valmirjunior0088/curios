...
- [ ] [An idiomatic string walk should cost what a hand-written one costs](roadmap/compiler/10_STRING_WALK_COST_SPEC.md) — `programs/parse_digits.crs` walks a string through `/std/Str/fold` and `programs/parse_manual.crs` indexes the bytes by hand, and the distance between them is a *sum*: the UTF-8 scan the control declines to do, one closure allocated per character because the fold's motive is a function, one indirect call per character through a parameter that never devirtualizes, and one transient `Option` per character from a lifted step. **The corpus that divides those four has been complete and unrun the whole time** — `programs/parse_bindless.crs` holds the scan constant and drops only the bind, exactly as `parse_manual.crs`'s header asks — yet `parse_bindless` and `parse_manual` appear in no probe, so two open items are each bounded above by the same undivided gap and each says its own share is unknown. So the first milestone is the division and the rest are chosen by it: one accumulator-threaded spelling in the library, then a written decision on whether Curios *promises* that both spellings of a dependently-typed left fold cost the same — because a thirteen-program corpus will report one site forever, and site-counting cannot answer the question that decides whether the compiler owes the transform.
...
- [ ] [A decided bound evaluates its subject, without a memory bound](roadmap/compiler/13_DECIDED_BOUND_EVALUATION_SPEC.md) — a precondition is discharged by reducing its subject, so a bound over a *computed* value runs that computation at elaboration time; the step budget bounds steps while nothing bounds the memory a reduction allocates, so at scale it exhausts the machine rather than refusing. Distinct from the unfolding discard above, which made such a bound unprovable rather than expensive and is fixed
- [x] Recursion restored to the defunctionalized walks (twelve walks — the kernel's `convert`/`infer`/`whnf`, `curios-elab`'s reducer, both universe-solver walks, the printer's document construction, the kernel's satisfiability search, the erased-module verifier, the size-change totality walk, the lowerer's `let` blocks, and the elaboration fast path — recurse inside `curios-base`'s `recurse` instead of driving explicit frame machines; `Term::walk` stays a shared driver and its internals are deferred indefinitely; `curios-cont`'s scoped CSE was reclassified as an undo log and had its record fixed to store a node rather than a derived key)
