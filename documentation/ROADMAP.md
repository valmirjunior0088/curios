...
- [x] Intrinsics as orthogonal builtins _(uniform `/sys` builtin declarations, not parser-special-cased)_
  ...
  - [x] `List`
- [ ] [Retiring `/sys`'s partial primitives, and the obligations that replace them](roadmap/compiler/11_TOTAL_PRIMITIVES_SPEC.md) — ten operations whose reduction can *fail*, in three families taking three different answers. `slice` clamps, the canonical extension `Nat/sub` already takes, which makes window fusion unconditionally true and so discharges the argued half of [SOUNDNESS.md](SOUNDNESS.md)'s intrinsic fold row by construction. `get` has no canonical extension — its result is an element, and a generic `T` may be uninhabited — so it keeps its `Nat/Lt` obligation and stops leaking the raw intrinsic past it, which is the gap `/std/Map` and `/std/List/find` both walked through. `div`/`rem` take their domain from a `Prop`-valued field on the `Div`/`Rem` concepts rather than the `x/0 = 0` junk value Lean and Isabelle adopt, whose reasoning inverts here: a proof erases, while a junk value costs a guard branch on every division since `i32.div_u` traps. Enabled by trivial inhabitation — an implicit whose type reduces to a nullary single-constructor proposition solved with that constructor, verified absent today and the one thing without which a proof-carrying primitive is unusable at a literal argument. Also re-scopes the `/sys` provenance gate onto ABI stability, correctness having moved into the types, and asserts the two Rust tables *A type is a pure term* rests on.
