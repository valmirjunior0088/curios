...
- [ ] [Restoring recursion to the defunctionalized walks](roadmap/compiler/11_STACKER_MIGRATION_SPEC.md) — explicit frame machines keep data-shaped depth off the native stack, which `curios-base`'s `recurse` now does without trading away the legibility two independently-written checkers depend on. Seven walks are restored — the kernel's three (`convert`, `infer`, `whnf`, −242/+131), then `curios-elab`'s reducer, both universe-solver walks, and the printer's document construction — leaving the lowerer's `let` sites and the elaboration fast path. `infer` established the stake: its worklist had silently dropped the three *checked* rules from every argument, payload and field position, and passed every gate for two weeks because the corpus never reached the shape. Three further machines were found outside the original inventory (`curios-analysis`'s totality walk, `curios-ersd`'s verifier, `curios-cont`'s CSE), each awaiting its own decision. `Term::walk` is explicitly out of scope and stays a shared driver.
