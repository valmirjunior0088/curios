...
- [ ] [You should pay for a value when you keep it, not when you name it](roadmap/compiler/14_VALUE_LIFETIME_COST_SPEC.md) — a value created and consumed inside one loop iteration, never escaping it, still costs a heap allocation and a runtime allocation call, and naming a value reads exactly like keeping one. What one character of an idiomatic string walk still costs is a byte read, the UTF-8 scan, an indirect call, and *two* allocations that never escape — one of them an accumulator tuple nobody wrote, which exists because the fold threads a pair. **Deliberately unrefined:** it states the thesis and the evidence and stops, because its predecessor's lesson was that naming a mechanism before measuring one is how a specification goes wrong. It also inherits the open question of whether Curios promises that both spellings of a dependently typed left fold cost the same
