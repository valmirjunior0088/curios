...
- [ ] [You should pay for a value when you keep it, not when you name it](roadmap/compiler/14_VALUE_LIFETIME_COST_SPEC.md) — a value whose identity nothing observes still costs a heap allocation and a runtime allocation call, and naming a value reads exactly like keeping one. What one character of an idiomatic string walk still costs is a byte read, the UTF-8 scan, an indirect call, and *three* allocations that are carried around the loop rather than escaping it — an accumulator tuple nobody wrote, a scan state destructured and rebuilt only to be passed along, and a rope view for the tail. **The mechanisms are named but the schedule is not:** each of the three falls to a different one, so the survey milestone chooses what gets built by measured yield, and is allowed to conclude that too little of it lives outside `/std` to be worth general machinery. It also inherits the open question of whether Curios promises that both spellings of a dependently typed left fold cost the same
