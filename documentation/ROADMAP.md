...
- [ ] [The prelude is an environment, not a prefix](roadmap/compiler/10_PRELUDE_ENVIRONMENT_SPEC.md) (the fixed prelude stops being copied into every user module and becomes an environment each stage is seeded from, deleting `Prefix`/`checked_from` and the prose caller contracts the certifier, elaboration and erasure each state and none check; then the archive's keying split, parallel per-item certification, and cached user modules)
- [ ] [One crate owns rkyv](roadmap/compiler/11_ARCHIVE_FACADE_SPEC.md) (a leaf `curios-archive` on the `curios-profile` pattern: it alone names rkyv, the root `[workspace.dependencies]` row goes, and an attribute macro replaces the four-line `cfg_attr` annotation at 127 type definitions)
