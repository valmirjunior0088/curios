...
- [ ] [A compilation is units folded over a dependency order](roadmap/compiler/10_COMPILATION_UNITS_SPEC.md) (the compiler stops structurally knowing there is a prelude and a program: one operation compiling a unit against a scope, `curios-pipeline` losing its `curios-prelude` edge, `RootId` opening, and `of_segment` deleted rather than extended — then cached units, then a manifest)
