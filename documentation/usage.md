...
What is written is `index.html`, one page per other module at its source path — `/json/parse/lexer` is `parse/lexer.html` — and, under `static/`, the stylesheet, the three brand fonts with their licenses, and the mark. Nothing else is fetched and no script runs, so the pages read from `file://`. Files are overwritten by name and nothing else in the directory is touched. A library that does not compile is not documented: its diagnostics are reported as `run` reports them, nothing is written, and the exit is non-zero. Success prints nothing.
