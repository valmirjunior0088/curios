...
5. **E — last, and its gate condition is already met.** `recheck_module` on the compile path, a refusal failing the compile, with the archive-verdict pattern so the prelude's verdicts ride the archive and only the entry suffix re-checks per compile. The pattern is mandatory, not an optimization, and the number that makes it so is measured: a `profile!`-spanned probe rechecking the full spliced module inside `compile_entrypoint` (release, memoized) costs **~3.6 s flat across hello, dependent_vectors, and binary_trees — 96% of a ~3.75 s total compile**, against ~150–190 ms for the entire rest of the pipeline; the flatness proves the cost is the ~1050 prelude items, so suffix-only rechecking projects to single-digit-to-tens of milliseconds per compile, below `lower_from_ersd`'s ~100 ms. The one-time prelude walk moves to the archive build. No other stage flagged: the remainder splits ~100 ms lowering, ~60 ms archive restore, ~40 ms elaboration.
