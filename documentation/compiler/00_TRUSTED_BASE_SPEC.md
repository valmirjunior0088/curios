...
**Measured after the split, D1, and D3's builder move:** `curios-cert` 6,181 non-test lines of judgments, `curios-core` 9,852 of representation (down from 15,065 holding both, with 515 lines of builders moved to `curios-elab`'s extension traits and the 64 `Flt` fold arms gone), plus `curios-base` 2,839, `curios-abi` 952, `num-bigint`, and the elaborator residue ~1,800 — roughly **21,600, inside the predicted band**. D3's builder move landed smaller than its ~1,400-line estimate because the census's import-following missed core-internal layering: the fold table passes comparison builders as rebuild functions, and the stay-wrappers (`nat_match`, `struct_type`) call the scoped variants, so 5 prim and 9 term candidates stayed home. What moved: 44 `Prim` builders and 9 `Term` builders, as `PrimBuilders`/`TermBuilders` extension traits in `curios-elab/src/builders.rs`, every call site keeping its spelling under an anonymous trait import. Still open in D3: the `universe.rs` solver-shaped strays, the unused `bin_match`/`into_parts` pair, and whether any of `term/tests.rs` follows its subjects.
...
**The kernel's obligation is local and self-derivable, not (T)/(V).** At both `rec` sites — `check_rec_group` for top-level groups, `infer`'s `Rec` arm for nested ones — a member whose declared type is `Prop`-sorted or yields a sort requires `group_totality == Total`, refusing as `NotDescending` otherwise. That closes `rec f : False = f` and its type-level twin `rec Bad : Type = Bad` (both fixtures landed, plus the control: a non-descending *value* recursion stays legal). **The residue is closed.** Partiality reached *indirectly* — a total-looking proof that calls a partial definition — was (T)/(V)'s reach analysis, which read the elaborator's settle records and stayed elaborator-only; it is now decided in `curios-cert` too, seeded from the kernel's own typing walk rather than from anyone's record (`curios-cert/src/obligation.rs`). Verified by disabling the elaborator's gate and watching the kernel refuse all four reach shapes on its own — direct, through an implicitly resolved witness, through a higher-order parameter, and through a function stored in a field — each naming its own offender. The whole prelude passes the new obligations, and the four fixtures are recorded in the two-checker matrix, where their kernel column read *accepts* before this landed.

**The cost fork is resolved, and not by any of its three named options.** The 10× blowup (16.6s → 164s) was attributed to cache-free reduction; instrumentation attributed it instead to **one group**: `/std/BigNat/add/raw_assoc`, whose 88 call matrices close to 1,599, at fifty seconds per closure — `close()` recomposed *closed × closed every round*. The fix is in the shared engine and pays both drivers: the closure now extends by **generators only** (every product is a shorter product followed by its last factor), in a hashed set — 50s → 22s → **0.88s** across the three steps, same 1,599-matrix result, verdicts byte-identical. The elaborator's archive build had been paying the same fifty seconds.

**The kernel additionally memoizes its own reductions**, with Lean's trusted `type_checker` as precedent (`m_whnf`, `m_whnf_core`, `m_unfold`, `m_infer_type`, `m_failure`): an unfold memo keyed by name (a definition body is closed), and whnf/forced memos keyed on local-free terms only — whnf reads no local and no constraint, so those reducts are functions of the definition store alone, and a redefinition clears every memo, making validity constructional. The line this draws in `Kernel`'s field docs: a metavariable heap or refinement layer *injects* answers a term alone could not produce; a memo replays the kernel's own pure function of `(term, definitions)`.

**A replay charges exactly what the computation charged — budget steps and minted identities — and `kernel_memo_parity` holds it to account.** The first parity run failed: hits that skip `spend` bend the budget trajectory, and recomputation mints eta-probe binders a replay does not, shifting every later identity in refusal payloads. Each memo entry therefore records its consumption and a hit charges both, exhausting exactly where the recomputation would have. The parity test runs the whole-prelude walk memoized and uncached and requires the verdict lists **bit-identical** — the property that makes this an evaluation strategy rather than a store, checked rather than asserted.

Final measurement: whole-prelude inventory **15.3s with the gate on** — under the 16.6s pre-gate baseline, the memos' item-walk saving covering the now-sub-second gate.
...
**C3 — Give the empty `Lst` literal an element type. Landed.** `Prim::Lst(Term, Vec<Term>)`, matching every other `Lst` form. The elaborator's metavar plumbing already existed — the empty literal minted an element-type metavariable and let unification ground it; the change stores what was minted, and the lowering supplies the fresh metavariable the literal cannot name. Every constructor site in reduction, the free-monoid peels, and the spine algebra had a locally available donor — `spine.rs` had even pre-built the element-threading (`lst_elem`) for slice windows, which the carried element makes total. The kernel's rule becomes the same shape as every other container: check the carried element is a type, check each element against it, `[]` included. `SCHEMA` bumped to 21. Measured: **38 of 1050 became 21** — the whole `Unclassified` class cleared, zero regressions. The premise this repairs is recorded in `DESIGN.md`'s synthesis-direction rationale: *"a finished Core term has no omitted annotations to recover"* was false for exactly one term form, and now is not.
...
**D1 — Make `Flt` opaque at the type level. Done.** The shared fold table's `Flt` family — arithmetic, comparisons, conversions in and out, the byte views — reduces operands and always rebuilds, in both checkers at once since the table is shared representation semantics. Literal-versus-literal conversion stays bitwise. `curios-ersd`'s partial evaluator already folded floats through `curios_base::scalar`, so runtime-faithful folding needed no move, only the trusted arms' removal. The decision record is `DESIGN.md`, "Flt is opaque at the type level"; the discipline is written on the fold table. The original sizing paragraph follows.

**D1, as originally specified.** The mechanism exists: `ReduceError::EffectAtTypeLevel` already refuses `Exit`, `Foreign`, and `Cell` (`reduce/prim.rs:1715-1739`). Constant folding moves to `curios-ersd`, where partial evaluation already lives and is untrusted. This is the recommended response to the divergence above, and its justification is correctness rather than line count — though ~64 fold arms and `curios-base/src/flt.rs` leaving the trusted base is a real side effect. The cost is a language decision: `refl : Eq(@Flt, 1.0 + 1.0, 2.0)` becomes unprovable. `/std/Flt.crs` is a two-line re-export facade with no lemmas and every prelude use of floats is codec work, so the corpus cost is zero.
...
**D2 — `print.rs`. Decided: it stays in `curios-core`, whole.** The decision was taken explicitly, as this entry asked, and the `curios-cert` split had already dissolved its premise: `curios-core` is now the representation crate rather than the judgment crate, so the printer sits beside the terms it renders and outside the crate whose rules admit programs — the enumeration's standing concession that printing cannot admit a bad program is unchanged. `KernelError`'s `Display` moved to `curios-cert` with its type, rendering through core's printer utilities in the sound dependency direction.
...
2. **The refusing-direction campaign — done, at zero.** 90 → 2 of 1050 by carving, then 2 → **0 of 1052** by moving the last two out of their refusal classes instead of weakening the kernel: `decode_head_raw` respelled through the explicit convoy (its strategy-coupled case-equation dependence retired), and `go_with`'s universe-instance drift closed in the elaborator by shape-equal defaulting at `add_eq` — measured by un-eliding the motive and watching the inventory hold at zero. The chase also surfaced and fixed a third elaborator defect the convoy tripped on: `solve` hard-failed any metavariable candidate whose weak-head form mentions a reducer-splayed `let` definition; the scope check now reifies such definitions back into the candidate.
...
5. **E — done.** `recheck_module_suffix` runs inside `compile_entrypoint` and a refusal fails the compile; the prelude prefix is defined on the archive's word, whose ground is `curios-prelude/build.rs` running the full walk at archive construction and failing the build on any refusal — an archive that exists is one whose every item the kernel accepted. The pattern was mandatory, not an optimization, and the number that made it so was measured first: a probe rechecking the full spliced module (release, memoized) cost **~3.6 s flat across hello, dependent_vectors, and binary_trees — 96% of a ~3.75 s total compile** — flat because the cost was the ~1050 prelude items; no other stage flagged (~100 ms lowering, ~60 ms archive restore, ~40 ms elaboration).

   Turning E on was itself the widest acceptance test the kernel had faced — every test program now compiles through it — and **8 of 436 programs surfaced gaps the three-fixture inventory never exercised**, in three classes. Five were the missing Σ-introduction rule: the kernel checked a tuple by inferring its components independently, manufacturing a non-dependent tuple type no conversion can relate to a dependent telescope; `check` now walks the expected telescope opened at the actual components, with the Π-introduction and `let`-descent rules landing alongside so a tuple under a lambda or a binding still meets its expectation (`check_fields`, `check_lambda`, and the `Let` arm in `infer.rs`). One was the recorded spine-at-`Type` incompleteness with its first live witness: `Eq(@P, p, q)` at `Prop`-sorted `P` needs irrelevance to fire at the index position, and an inductive type-former's arguments are now compared at the declaration's own index telescope (`induct_type_args`) — the `Apply`-spine, stuck-elimination, and struct-parameter positions stay grounded, no witness having forced them. Two were the tests' own fault, and the kernel was right: `.with_type("()")` declared the unit *value* as an entry type, which has no sort — corrected to `"{}"`. That the elaborator accepted a value in type position is an open elaborator finding. All 436 tests, the zero-refusal inventory, and memo parity hold with the rules in.
...
After C3's carried element type: **21 of 1050** — the 17-item `Unclassified` class cleared entirely, zero regressions. What remains is the tail: projection-shaped inversions, the four syntactically-compared conversion positions, and a few singletons needing individual diagnosis.
...
After the vacuous-elimination rule (an arm-less, default-less match skips the guard; the coverage loop must then prove every constructor impossible, so the eliminated instance is uninhabited): **still 52** — `/std/Str/at`'s `LargeElimination` refusal became the `Utf8` index mismatch its siblings already show, which is the guard hole closed and the conversion gap left standing.

After the case-equation registry (`Kernel::refinements`: within an arm the scrutinee expression *is* the case's value, definitionally — the built-in convoy face, scoped through `mark`/`retract`, consulted by `whnf` at stuck heads; keys must mention a local, which is what keeps the local-free evaluation memos sound; a scrutinee that refuses type-level reduction records its equation at its written spelling rather than refusing the item): **52 became 47** — `Char/of_nat`, both `cont0`/`cont_len` `Utf8` items, `BigNat/view`, and `is_trimmed_of_canonical` cleared, zero regressions.

After the outer-direction relaxation (`invert_indices_outer`: in the second specialization direction the flex variables are the context's, not an arm's, so a variable actual may solve the flex target it meets — the kernel's spelling of `refine_head`'s orientation; the first direction keeps its guard): **47 became 45** — `/std/Eq/subst` and `cong_cons2` cleared, the repeated-binder shape whose two directions had each been deferring the variable-variable case to the other.

After stuck-elimination motives and arms compare up to conversion (`ground_scope`/`ground_cases`: both scopes opened at shared opaque binders assumed at `Type` — sound because `ground` is already the untyped concession, so binder types feed only the history key — with tags, plicities, arity, and default presence held rigid): **45 became 40** — `Str/at`, `Str/fold`, and `peel_byte` cleared or progressed past their `Utf8` mismatches, and five `cmp` items moved. The witnessing shape: `step(c, st)` against `step(at(cons(c, t), 0, _), st)` both delta-reduce to stuck matches on `st` whose arms are convertible but not identical.

After the unfold-spelling rule (`unfold_spelling` in `whnf.rs`, consulted by `structural`'s catch-all: `force` keeps a recursive call folded as its normal form while an arm's induction hypothesis is the raw stuck fold-match on the same argument — when heads disagree, each side is granted the one definitional unfolding `force` withheld and the results compared, cycles bounded by the recurrence rule): **40 became 24**, the campaign's largest drop — the whole fold-versus-name family, `trim_head_*`, `of_scan_eq_from`, `go_with`, and, notably, most of the apparent `Eq.{0}`-vs-`Eq.{u}` level class, whose mismatches were downstream symptoms of the spelling gap rather than universe issues. The walk also got faster (earlier convergence).

After the spine-retry rule (a failing same-shape application comparison — two applications of one fold differing in a position the fold discards, `is_trimmed(h ++ rest)` against `is_trimmed(rest)` — retries through the same one-step unfolding, bounded at `RETRY_DEPTH` because a retried spine may grow and the coinductive rule then cannot close the chain): **24 became 5**, nineteen cleared, zero regressions — the whole `is_trimmed` cluster, the deep rec-form family, `Str/at`, and `of_scan_eq_from`.

**The retry exposed, and the countdown fixture caught, a latent unsoundness in the conversion history: it was a seen-set, not a path.** A goal refuted in one subtree lingered in `seen`, and when a later subtree re-derived the same goal the recurrence rule read the stale entry as an in-progress cycle and assumed it held — `x ~ y` refuted under a spine comparison was then assumed while comparing the unfolded matches' heads, converting two different recursive calls. `History::enter`/`leave` now hold exactly the goals on the current path, removed on completion *whatever the outcome*, because a goal proven under in-progress assumptions is not a fact once they are gone. `a_folded_recursive_call_converts_without_unfolding_forever` is the fixture that caught it, doing precisely the job its name promises.
...
**The campaign closes at zero: 0 of 1052 (1053 arithmetic), all three profiles.** The final two moved out of their classes rather than being carved. `decode_head_raw` was respelled through the explicit convoy — `scan_of_class` on the class, a `refl` lemma `step_scans` stating `step(c, lead()) = scan_of_class(classify(c))`, and a motive-abstracted match whose arms receive the transported witness at their own scan — after the registry's entry-probe extension was tried and reverted (β-substitution respells occurrences past any finite key set; the coupling is now recorded in `DESIGN.md`'s kernel entry). `go_with` closed on the elaborator's side: `add_eq` now defaults a shape-equal equation at its single differing metas, measured by un-eliding the motive and watching the inventory hold at zero, with the elision kept as preferred style. Landing the convoy surfaced a third elaborator defect, diagnosed by instrumented probe and fixed: `solve` used the weak-head form as the candidate solution, `reduce_let` splays a `let` into an entropy-fresh context definition, and the scope check hard-failed the splayed name — so no metavariable could be solved against any term whose whnf forces a `let`, exactly the shape proof-carrying index positions produce. The scope check now reifies such definitions back into the candidate; the regression fixture is the refused shape verbatim, and the inventory re-held at zero after the fix.
...
**Turning the kernel on doubles the checking work per module.** Partly measured, and materially smaller since the kernel memoizes its own reductions (see A4): the whole-prelude walk runs in ~15s with every gate on. A3 settles positivity's share by counting the fixpoint domain; A4's is the open one. The fallback is neither a flag nor a weakening but the pattern `positivity.rs:66-76` already uses — compute at archive-build time, read at replay — which under the amended principle reads the kernel's own answer.
...
**A 10× cost was attributed to cache-free reduction, and profiling attributed it to one loop.** Two memoization designs were built for it — a per-walk force memo (no effect, reverted) and the session memos above (~20% on the item walk, kept) — before instrumentation found fifty of the hundred-and-fifty seconds in a single group's `close()`. The design discussions were not wasted, but the order was wrong, and the repo already owned the instruction being relearned: profile before designing, and the profile is per-item, not per-theory.
