...
**~~The erasure premise is stated, not demonstrated.~~** **Closed, and it was misstated.** Erasure deletes strictly more than types and `Prop`-sorted proofs: three of its five deletion sites drop a whole construct including its arguments, which are ordinary values, and it skips dead code after a terminator for reachability rather than for sort. The premise that matters is a *containment* — everything deleted lies within a term (T) or (V) covers — which holds because the closure and the local-partiality walk descend through whole terms, and because the obligations are reachability-blind. One fixture per site in `tests::soundness`. See "The erasure premise" in `DESIGN.md`.

**~~(T) is a walk, and walks are what this effort kept finding defects in.~~** **Closed, and the walk had a gap.** Attacking it found one: `annotations` has no case for an application's parameters, so a type passed as an *argument* was seeded nowhere, and `ignore(@Shape(inf), 5)` — a total type-level function applied to a partial value — was accepted while the same type in an annotation was not. (T) now unions the walk with the terms elaboration settled *at a sort*, which catches argument positions by construction. The two halves are incomparable rather than nested, which comparing their counts had obscured. The corpus paid nothing and the prelude hash-consed unchanged.
