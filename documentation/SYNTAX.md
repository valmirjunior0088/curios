...
**Private items stay out of public interfaces.** A publicly exposed nominal type must only mention items publicly reachable from that interface. Parameters, indices, and the result sort belong to the nominal interface; struct fields and inductive constructor signatures join it only when the declaration-local `pub` exposes the representation. The audit runs after `pub use` resolution and follows identity aliases plus direct-headed type-family aliases: after peeling the definition's function binders, its declared result must be the literal `Type` or `Prop`, and its body must be an application spine headed by an absolute item name. This includes specialization, argument reordering, and constant families without evaluating arbitrary terms; matches, `let`s, projections, beta-redexes, local function heads, and aliases whose universe annotation is itself an alias stay outside the policy. A private-name/public-representation declaration may use private helpers while hidden but must expose those helpers, including dependencies supplied in a followed alias body, if a facade later exposes its representation. Ordinary public definition bodies remain exempt and this is not computational sealing.
