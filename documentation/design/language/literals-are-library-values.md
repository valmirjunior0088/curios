...
**Decision.** Character and string literals lower to transparent `/std/Char` and `/std/Str` values — proof-certified library structures — while the erased runtime carriers remain `Nat` and packed `Bytes`.
