...
**Decision.** The zero-arm match is the language's one way to discharge a proof of an empty type: `match contradiction end` in checking position, `match contradiction : (_) => A end` where the result must be spelled. The library exports no `absurd`; every former call site in `/std` is spelled as a zero-arm match.

**Rationale.** The match is the primitive `absurd` merely wrapped, and it is strictly more capable: it eliminates any type that *reduces* to an empty inductive under the arm's refinements — the shape of `/std/Str/Valid/drop`'s empty arms, where the proof's type becomes `False` only inside the arm — which the function cannot accept without an explicitly supplied, refinement-dependent implicit that resolution deliberately refuses to synthesize. The deleted `/std/False/absurd` also violated the `/syn` law: the compiler never emitted the name, and `/syn` membership means exactly that Rust lowering does.
