...
**Rejected.** Holding both the initializer and the eventual result in one channel would require tagged wrapper values to distinguish them and an observing operation to read the cached result without consuming it. The cell-and-channel pair keeps the initializer consumable and the result persistent using the existing operations, without those wrappers.
