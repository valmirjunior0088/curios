...
impl Harvest {
    ...
    fn decide(mut self) -> HashMap<ValueName, ValueName> {
        // Join edges into slots.
        let edges = std::mem::take(&mut self.edges);
        for (target, args) in &edges {
            let Some(&(start, end)) = self.blocks.get(target) else {
                continue;
            };

            let slots = &mut self.slots[start..end];
        ...
        for target in &self.resumes {
            if let Some(&(start, end)) = self.blocks.get(target) {
        ...
        loop {
            ...
            for slot in &self.slots {
                ...
                for arg in &slot.args {
                    let arg = resolve(arg, &self.aliases, &decided);
                    ...
                    if self.slots.iter().any(|s| s.param == arg) && !decided.contains_key(&arg) {
                        opaque = true;
                        break;
                    }
                ...
                if !in_scope(&value, &slot.path, &self.bound) {
                    continue;
                }
