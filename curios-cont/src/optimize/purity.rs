...
impl BodyScan {
    /// Whether this scanned body's edges reach an already-impure func or clsr.
    fn propagates(
        &self,
        impure_funcs: &HashSet<FuncName>,
        impure_clsrs: &HashSet<ClsrName>,
    ) -> bool {
        self.func_calls.iter().any(|t| impure_funcs.contains(t))
            || self.clsr_refs.iter().any(|c| impure_clsrs.contains(c))
    }
}
...
fn classify(module: &Module) -> (HashSet<FuncName>, HashSet<ClsrName>) {
    ...
    loop {
        ...
        for (name, scan) in &func_scans {
            ...
            if scan.propagates(&impure_funcs, &impure_clsrs) {
