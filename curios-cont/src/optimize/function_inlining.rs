...
/// Count the items in a function's region tree — [`Region::size`]
...
fn freshen_tail_blocks(tail: &mut Tail, sentinel: &BlockName, resume: &BlockName, suffix: &str) {
    match tail {
        ...
        Tail::Cell(cell) => {
            freshen_block(cell.resume_mut(), sentinel, resume, suffix);
        }
        Tail::Unreachable => {}
    }
}
...
mod tests {
    ...
    fn func_named<'a>(module: &'a Module, name: &str) -> Option<&'a Func> {
        module.find_func(&FuncName::from(name))
    }
    ...
    fn inlines_chain_after_dce() {
        ...
        // Pipeline order from src/cont/optimize.rs::optimize.
