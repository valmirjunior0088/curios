...
impl<'a> Interp<'a> {
    ...
    fn bind_preallocs_and_values(region: &Region, frame: &mut Frame) -> Result<(), Outcome> {
        ...
        // are prealloc'd (cyclic tuples/arrays are rejected upstream in `into_cont`).
    ...
    fn eval_value(value: &Value, frame: &Frame) -> Option<Snapshot> {
        match value {
