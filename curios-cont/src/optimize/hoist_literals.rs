...
/// `Lst`, `struct.new` for a `Tpl`/`Clsr` (`cont/into_wasm/expr_emitter.rs`). The same
...
/// (`cont/into_wasm/module_emitter.rs`). For a constant in a loop, a closure body, or
...
/// are emitted as `global.get` of other consts (`cont/into_wasm/context.rs`), with no
