...
///
/// Handed to a client's round closure by [`solve`]. Reads see every join performed earlier in the same round, which is deliberate: a chaotic iteration reaches the same least fixpoint as a round-synchronised one but typically in fewer rounds, and the fixpoint is what the client's answer is defined as.
pub(super) struct Solver<F> {
...
impl<F: Lattice> Solver<F> {
    ...
    pub(super) fn facts(&self) -> &BTreeMap<CpsValueId, F> {
        &self.facts
    }
    ...
    pub(super) fn join(&mut self, id: CpsValueId, incoming: F) {
...
pub(super) fn solve<F: Lattice>(
    keys: impl IntoIterator<Item = CpsValueId>,
    mut round: impl FnMut(&mut Solver<F>),
