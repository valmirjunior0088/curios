...
//!
//! The names below join the flat `cps` namespace rather than being reached through this module, so they are spelled for that namespace: [`Solver::solve`] is the entry point rather than a bare `solve`, which would be far too general a name to sit beside `Module`.

use {super::ValueId, std::collections::BTreeMap};
...
impl<F: Lattice> Solver<F> {
    ...
    pub(crate) fn join(&mut self, id: ValueId, incoming: F) {
    ...
    pub(crate) fn solve(
        keys: impl IntoIterator<Item = ValueId>,
