...
pub(super) fn evaluate(op: CpsIntrinsicOp, args: &[CpsAtom]) -> Option<CpsLiteral> {
    ...
    match op {
        ...
        CpsIntrinsicOp::NatLe => bool_(nat(0)? <= nat(1)?),
        CpsIntrinsicOp::NatGe => bool_(nat(0)? >= nat(1)?),
        ...
        CpsIntrinsicOp::IntLe => bool_(int(0)? <= int(1)?),
        CpsIntrinsicOp::IntGe => bool_(int(0)? >= int(1)?),
        ...
        CpsIntrinsicOp::FltLe => bool_(flt(0)? <= flt(1)?),
        CpsIntrinsicOp::FltGe => bool_(flt(0)? >= flt(1)?),
