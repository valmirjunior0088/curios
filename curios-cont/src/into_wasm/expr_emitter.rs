...
impl<'a, 'b> ExprEmitter<'a, 'b> {
    ...
    pub(crate) fn emit_data(&mut self, value_name: &'a EmissionValueName, value: &'a EmissionData) {
        match value {
            ...
            // Each slot is filled at the carrier its row declares: a scalar goes in raw, a shaped reference through a null-admitting cast — the null a padded slot travels as is exactly what the field holds — and a slot this constructor does not write takes that slot's own zero. A head rebuild is an ordinary construction: the field parameter it stores is the slot's value or its null, and nothing else can reach it.
