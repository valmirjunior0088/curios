...
fn convert(name: &FuncName, func: &mut Func) {
    let counts = count_self_calls(&func.region, name, &func.resume);
    if counts.tail == 0 || counts.non_tail > 0 {
        return;
    }

    let header = mangle::loop_header();
    rewrite_self_calls(&mut func.region, name, &header);
    ...
    // keys on them.
    let header_params: Vec<ValueName> = func.params.iter().map(|p| p.name.clone()).collect();
    let outer: Vec<Argument> = func
        .params
        .iter()
        .map(|param| Argument {
