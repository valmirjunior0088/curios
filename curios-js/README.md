...
The Curios ↔ JavaScript boundary: a `wasm-bindgen` export of [`curios-pipeline`](../curios-pipeline) (`compile`) plus the browser run harness (`run`, with `bridge_bytes` and `abi` as its exported building blocks), built for `wasm32-unknown-unknown` with `cargo build` + `wasm-bindgen-cli --target web` for the browser playground (see AGENTS.md's Gotchas for why not `wasm-pack`).
