...
//!
//! So the recursion stays and the stack grows to fit it: [`recurse`] is that bracket, and the only place the figures are written. Why recursion rather than an explicit frame stack, and why the figures live once, are `README.md`'s decisions.
//!
