//! What a Curios package is, and everything that reads one: the `curios.toml` manifest, the walk that decides which manifest governs an invocation, the resolver that turns a declared dependency into a module tree, and the store the results are filed in.
//!
//! **One subsystem, one owner.** Those four are faces of a single question — what is in this compilation, and where did each part come from — so they are one crate rather than four seams through the driver. `curios-pipeline` stays pure and never learns this crate exists: it folds its stages over whatever scope it is handed, and *deciding* that scope is a product's job, exactly as supplying the syntax registry already is. `curios` is the product that decides it, over this crate. `curios-js` never touches it, which is the constraint that keeps a resolver from quietly assuming a filesystem.
//!
