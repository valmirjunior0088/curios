...
// The syntax registry stand-in every kernel these tests build is handed. Shared with `curios-elab` and with `curios-analysis`'s own suite rather than copied into each, and gated there behind `test-support` so it reaches no build that ships.
#[cfg(test)]
