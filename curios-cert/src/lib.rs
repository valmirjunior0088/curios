...
//!
//! This crate holds every rule that can admit a program — the kernel deciding, from a finished term alone, whether it is well-typed, and the analyses both checkers share: index inversion and the singleton determination walk ([`invert_indices`], [`pinned_by_targets`]), strict positivity ([`positivity_vectors`]), size-change totality ([`group_totality`]), and the level entailment oracle ([`entails`]). What a term *is* — the representation, its binder discipline, the primitive roster and its folds — belongs to `curios-core`, which this crate builds on and which is the only thing it shares with the elaborator: sharing the representation is not sharing a judgment.
//!
