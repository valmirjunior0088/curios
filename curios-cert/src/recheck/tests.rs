...
///
/// The two forms failed differently, and neither failed well. A `Struct` opens the declaration's arity with `Telescope::open`, which **asserts** — so a value at no parameters for a one-parameter structure aborted the process with `telescope arity mismatch in open: expected 1, got 0`, and at two parameters with `expected 1, got 2`. A panic refuses rather than admits, so it is inside what `documentation/soundness/across-the-perimeter.md` permits of the Rust implementation, but it is the wrong shape twice over: a malformed value is the *program's* fault, which the house rule says is a `KernelError`, and `recheck_module_verdicts` is documented as walking to the end with each verdict independent of the others — an abort takes every other verdict with it, which is what makes the disagreement count a count.
///
...
fn already_judged(module: &Module) -> Globals {
    Globals::of(module, derived_binder_floor(module))
}

/// A module carrying `items` and `induct_decls` and nothing else.
fn collision_module(items: Vec<Item>, induct_decls: BTreeMap<Global, InductDecl>) -> Module {
    Module {
        mounts: Vec::new(),
        items,
        universe_seeds: Vec::new(),
...
fn shadowing_registry(payload: Term) -> Module {
    collision_module(
        Vec::new(),
        BTreeMap::from([(
            shadowed_family(),
