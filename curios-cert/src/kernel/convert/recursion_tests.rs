...
///
/// Instrumenting `History::enter` and running the whole corpus — the fixed prelude through the kernel's own walk, plus every test program — counts 83,945 goals entered and **zero** recurrences. The two recursive-group fixtures above do not reach it either: `two_alpha_variant_recursive_groups_convert` converges because alpha-variant groups are structurally equal, and `a_folded_recursive_call_converts_without_unfolding_forever` terminates on `force` discarding an unfolding that restuck. So the rule was live code that no program could exercise, which is the condition under which a rule's mistakes stay invisible, and the corpus passed throughout.
///
