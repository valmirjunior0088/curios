...
//!
//! This crate cannot see `curios-prelude` — the dependency runs the other way, which is the whole reason the registry is handed to `Kernel::new` rather than spelled inside it — so its unit tests need their own values, exactly as `curios-elab`'s and `curios-text`'s do. Nothing here is authoritative: no test in this crate resolves one of these names, they exist so a kernel can be built at all, and production compilation never constructs it.
